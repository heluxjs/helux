...
function getFragmentClassKeyAndStpMapping(connectSpec, fragmentModule) {
  if (!util.isObjectNotNull(connectSpec) && fragmentModule == MODULE_DEFAULT) {//代表没有connect到store任何模块的CcFragment
    return { ccClassKey: `${CC_FRAGMENT_PREFIX}_0`, stateToPropMapping: null };
  }

  const { featureStr, stateToPropMapping, connectedModuleNames } = getFeatureStrAndStpMapping(connectSpec, fragmentModule);
...
export default class CcFragment extends Component {
  constructor(props, context) {
    ...
    const { ccClassKey, stateToPropMapping, connectedModuleNames } = getFragmentClassKeyAndStpMapping(connectSpec, fragmentModule);
    ...
    const ccState = {
      storedStateKeys: _storedStateKeys, sharedStateKeys: _sharedStateKeys,
