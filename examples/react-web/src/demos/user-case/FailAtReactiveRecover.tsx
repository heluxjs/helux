...
function App() {
  ...
  return (
    <>
      <h1>Vite + React</h1>
      <div className="card">
        <button onClick={change}>count is {count}</button>
      </div>
      <div className="read-the-docs">
        <button onClick={() => (reactive.a.b.c = 9999)}>reactive 恢复 firstName默认值</button>
        <button onClick={recover}>setState 恢复 firstName默认值</button>
        <button onClick={changeReactive}>changeReactive</button>
        <div>
          firstName=
          <input value={state.a.b.c} onChange={sync((to) => to.a.b.c)} />
        </div>
      </div>
    </>
  );
