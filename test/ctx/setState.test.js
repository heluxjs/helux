...
import { okeys, delay } from '../../src/support/util';
import { getTestModels, getWrapNum } from '../util';
...
describe('test ctx api setState', () => {
  const mountThenClickFor1Param = (CompFn, CompCls) => {
  ...
  test('pass empty state should do nothing', () => {
    ...
    const {
      compFnRenderCount, compClsRenderCount,
      compFnRenderCountAfterClick, compClsRenderCountAfterClick,
    } = mountThenClickFor1Param(CompFn, CompCls);
  ...
  test('pass valid state should trigger comp-ins re-render', () => {
    const { CompFn, CompCls } = makeTestComp((/** @type Ctx*/ctx) => {
      return () => ctx.setState({ name: Date.now() });
    });
    const {
      compFnRenderCount, compClsRenderCount,
      compFnRenderCountAfterClick, compClsRenderCountAfterClick,
  ...
  test('renderKey should work', async () => {
    const makeClickHandler = (/** @type Ctx*/ctx) => () => {
      ...
      book.author = 'epc';
      ctx.setState({ books }, null, bookId);
    };
  ...
  test('renderKey and delay should work for item click', async () => {
    const makeClickHandler = (/** @type Ctx*/ctx) => () => {
      ...
      const book = books.find(v => v.id === bookId);
      book.author = 'monster';
      ctx.setState({ books }, null, bookId, 500);
    };
