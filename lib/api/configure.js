...
function _default(module, config) {
  ...
  if (!isPlainJsonObject(config)) {
    throw new Error("[[configure]] config " + _privConstant.NOT_A_JSON);
  }
  ...
  if (reducer && !isPlainJsonObject(reducer)) {
    throw new Error("[[configure]] config.reducer " + _privConstant.NOT_A_JSON);
  }

  (0, _initModuleState["default"])(module, state, true);
  (0, _initModuleReducer["default"])(module, reducer, true);
