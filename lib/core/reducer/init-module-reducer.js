...
function _default(module, reducer) {
  ...
  var subReducerCaller = util.safeGetObjectFromObject(_caller, module); // const subReducerRefCaller = util.safeGetObjectFromObject(_reducerRefCaller, module);

  var fnNames = util.safeGetArrayFromObject(_module_fnNames_, module); // 自动附加一个setState在reducer里
  ...
  reducerNames.forEach(function (name) {
    ...
    var list = util.safeGetArrayFromObject(_fnName_fullFnNames_, name); // avoid hot reload
