...
function _default(module, reducer, rootReducerCanNotContainInputModule) {
  ...
  var subLazyReducerCaller = util.safeGetObjectFromObject(_lazyReducerCaller, module);
  var fnNames = util.safeGetArrayFromObject(_reducerModule_fnNames_, module);
  var reducerNames = util.okeys(reducer);
  reducerNames.forEach(function (name) {
    fnNames.push(name);

    subReducerCaller[name] = function (payload, delay, idt) {
      return (0, _dispatch["default"])(module + "/" + name, payload, delay, idt);
    };

    subLazyReducerCaller[name] = function (payload, delay, idt) {
      return (0, _lazyDispatch["default"])(module + "/" + name, payload, delay, idt);
    };
    ...
    var list = util.safeGetArrayFromObject(_reducerFnName_fullFnNames_, name);
