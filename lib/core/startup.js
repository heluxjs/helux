...
function bindComputedToCcContext(computed, isModuleMode) {
  ...
  if (isModuleMode) {
    ...
    var includeDefaultKey = computed.hasOwnProperty(_constant.MODULE_DEFAULT);
    var includeGlobalKey = computed.hasOwnProperty(_constant.MODULE_GLOBAL);
    ...
    if (includeDefaultKey || includeGlobalKey) {
      ...
      if (includeDefaultKey) {
        invalidKeyCount++;
        mapComputed(_constant.MODULE_DEFAULT, computed[_constant.MODULE_DEFAULT]);
      }

      if (includeGlobalKey) {
        invalidKeyCount++;
        mapComputed(_constant.MODULE_GLOBAL, computed[_constant.MODULE_GLOBAL]);
      }
  ...
  try {
    ...
    if (_ccContext["default"].isCcAlreadyStartup) {
      ...
      if (_util["default"].isHotReloadMode()) {
        ...
        (0, _util.clearObject)(_ccContext["default"].moduleName_ccClassKeys_);
