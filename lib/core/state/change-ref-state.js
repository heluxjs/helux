...
function _default(state, _temp, targetRef) {
  ...
  var passedRef = stateFor === FOR_ONE_INS_FIRSTLY ? targetRef : null; // 标记noSave为true，延迟到后面可能存在的中间件执行结束后才save
...
function triggerReactSetState(targetRef, callInfo, renderKey, calledBy, state, stateFor, reactCallback, next) {
  ...
  var shouldCurrentRefUpdate = (0, _watchKeyForRef["default"])(targetRef, stateModule, refState, deltaCommittedState, callInfo, false, true);
...
function broadcastState(callInfo, targetRef, partialSharedState, stateFor, moduleName, renderKey) {
  ...
  connectRefKeys.forEach(function (refKey) {
    ...
    if (ref.__$$isUnmounted !== true) {
      var refCtx = ref.ctx;
      (0, _computeValueForRef["default"])(ref, moduleName, prevModuleState, partialSharedState, callInfo);
      var shouldCurrentRefUpdate = (0, _watchKeyForRef["default"])(ref, moduleName, prevModuleState, partialSharedState, callInfo); // 记录sharedStateKeys，方便triggerRefEffect之用
