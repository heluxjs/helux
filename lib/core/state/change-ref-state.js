...
var _ccContext$store = _ccContext["default"].store,
    ...
    renderKey_ccUkeys_ = _ccContext["default"].renderKey_ccUkeys_; //触发修改状态的实例所属模块和目标模块不一致的时候，stateFor是FOR_ALL_INS_OF_A_MOD
...
function triggerReactSetState(targetRef, callInfo, renderKey, calledBy, state, stateFor, reactCallback, next, skipMiddleware, passToMiddleware) {
  ...
  if (targetRef.__$$isUnmounted === true || stateFor !== FOR_ONE_INS_FIRSTLY || //确保forceUpdate能够刷新cc实例，因为state可能是{}，此时用户调用forceUpdate也要触发render
...
function broadcastState(callInfo, targetRef, partialSharedState, stateFor, moduleName, renderKey) {
  ...
  var ignoreCurrentCcUkey = stateFor === FOR_ONE_INS_FIRSTLY; // these ccClass are watching the same module's state
  ...
  if (renderKey) {
    ...
    var ccUkeys = util.excludeArrStringItem(ccUkeysOri, currentCcUkey);
