...
exports.makeModuleDispatcher = makeModuleDispatcher;
exports.makeSetStateHandler = makeSetStateHandler;
exports.makeRefForceUpdate = exports.makeRefSetState = void 0;
...
var _dispatch2 = _interopRequireDefault(require("../base/dispatch"));
...
function handleCcFnError(err, __innerCb) {
  if (err) {
    if (__innerCb) __innerCb(err);else {
      justWarning(err);
      if (_ccContext["default"].runtimeHandler.errorHandler) _ccContext["default"].runtimeHandler.errorHandler(err);
    }
  }
}
...
function invokeWith(userLogicFn, executionContext, payload) {
  ...
  isStateModuleValid(targetModule, callerModule, cb, function (err, newCb) {
    ...
    if (context) {
      ...
      var lazyDispatch = makeDispatchHandler(callerRef, true, isSilent, targetModule, renderKey, delay, chainId, oriChainId, chainId_depth_); //oriChainId, chainId_depth_ 一直携带下去，设置isLazy，会重新生成chainId
      ...
      actionContext = {
        ...
        setState: function setState(state, r, d) {
          var targetR = r !== 0 ? r || renderKey : r;
          var targetD = d !== 0 ? d || delay : d;
...
function dispatch(_temp2) {
  ...
  if (!reducerFn) {
    var fns = okeys(targetReducerMap);
...
function makeDispatchHandler(callerRef, in_isLazy, in_isSilent, defaultModule, defaultRenderKey, delay, chainId, oriChainId, chainId_depth_ // sourceModule, oriChainId, oriChainDepth
  ...
  return function (paramObj, payload, userInputRKey, userInputDelay) {
    ...
    if (_module === '*') {
      return (0, _dispatch2["default"])("*/" + _type, payload, {
        silent: isSilent,
        lazy: isLazy,
        renderKey: _renderKey
      }, _delay, {
...
function makeModuleDispatcher(module) {
  return function (action) {
    ...
    _dispatch2["default"].apply(void 0, [_action].concat(args));
  };
...
function makeSetStateHandler(module, initStateDone) {
  return function (state) {
    ...
    try {
      (0, _setState.innerSetState)(module, state, execInitDone);
    } catch (err) {
      ...
      var _extractStateByKeys = (0, _extractStateByKeys2["default"])(state, keys, false, true),
...
exports.makeRefSetState = makeRefSetState;
...
exports.makeRefForceUpdate = makeRefForceUpdate;