...
var getWatchedKeys = function getWatchedKeys(ctx) {
  if (ctx.watchedKeys === '-') return okeys(ctx.__$$curWaKeys);else return ctx.watchedKeys;
};
...
var getConnectWatchedKeys = function getConnectWatchedKeys(ctx, module) {
  ...
  var getWKeys = function getWKeys(module) {
    if (isConnectArr) {
      ...
      return okeys(ctx.__$$curConnWaKeys[module]);
    } else {
      var waKeys = connect[module];
      if (waKeys === '*') return moduleName_stateKeys_[module];else if (waKeys === '-') return okeys(ctx.__$$curConnWaKeys[module]);else return waKeys;
    }
  };
...
function _default(ref, params, liteLevel) {
  ...
  ctx.__$$reInjectConnObState = function () {
    ...
    if (Array.isArray(connect)) {
      connectedModules.forEach(function (m) {
        connectedState[m] = (0, _makeObState["default"])(ref, classConnectedState[m], m);
      });
    } else {
      connectedModules.forEach(function (m) {
        var waKeys = connect[m];
        if (waKeys === '-') connectedState[m] = (0, _makeObState["default"])(ref, classConnectedState[m], m);else connectedState[m] = classConnectedState[m];
      });
    }
