...
var _ccContext$reducer = _ccContext["default"].reducer,
    ...
    waKey_effectUKeyMap_ = _ccContext["default"].waKey_effectUKeyMap_;
...
function _default(ref, params, liteLevel) {
  ...
  var privStateKeys = util.removeArrElements(okeys(state), moduleName_stateKeys_[stateModule]); // record ccClassKey
      ...
      return function (fn, depKeys, compare, immediate) {
        ...
        if (!isProp && _depKeys) {
          ...
          _depKeys.forEach(function (depKey) {
            ...
            var map = safeGet(waKey_effectUKeyMap_, modDepKey);
            map[ccUniqueKey] = 1;
          });
  ...
  if (watchedKeys === '-') {
    __$$autoWatch = true;
    ref.state = (0, _makeObState["default"])(ref, mergedState, module, true);
    ctx.state = ref.state;
  } else {
    //开始记录依赖
