...
function _default(ref, params, liteLevel) {
  ...
  var setModuleState = function setModuleState(module, state, reactCallback, delay, renderKey) {
    _setState(module, state, _constant.SET_MODULE_STATE, reactCallback, delay, renderKey);
  ...
  var forceUpdate = function forceUpdate(reactCallback, delay, renderKey) {
    _setState(stateModule, ref.state, _constant.FORCE_UPDATE, reactCallback, delay, renderKey);
  };
  ...
  if (liteLevel > 1) {
    ...
    ctx.lazyInvoke = hf.makeInvokeHandler(ref, ccKey, ccUniqueKey, ccClassKey, {
      isLazy: true
    });

    ctx.setGlobalState = function (state, reactCallback, delay, renderKey) {
      _setState(_constant.MODULE_GLOBAL, state, _constant.SET_STATE, reactCallback, delay, renderKey);
    };
  }
