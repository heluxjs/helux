...
var _extractor = require("../param/extractor");
...
var okeys = util.okeys,
    ...
    isDepKeysValid = util.isDepKeysValid,
    isObject = util.isObject,
    isBool = util.isBool,
    justWarning = util.justWarning,
    isObjectNull = util.isObjectNull,
    isValueNotNull = util.isValueNotNull,
    noDupPush = util.noDupPush;
var idSeq = 0;
...
var getConnectWatchedKeys = function getConnectWatchedKeys(ctx, module) {
  ...
  var getWKeys = function getWKeys(module) {
    if (isConnectArr) {
      ...
      if (waKeys === '*') return getModuleStateKeys(module);else if (waKeys === '-') return getModuleWaKeys(module);else return waKeys;
    }
  };
...
function recordDep(ccUniqueKey, module, watchedKeys) {
  var waKeys = watchedKeys === '*' ? getModuleStateKeys(module) : watchedKeys;
...
function _default(ref, params, liteLevel) {
  ...
  var modStateKeys = getModuleStateKeys(stateModule);
  ...
  var ccUniqueKey = (0, _computeCcUniqueKey["default"])(ccClassKey, ccKey, refOption.tag); // 没有设定renderKey的话读id，最后才默认renderKey为ccUniqueKey

  refOption.renderKey = ccOption.renderKey || id || ccUniqueKey;
  refOption.storedKeys = (0, _extractor.getStoredKeys)(stateModule, state, ccOption.storedKeys, storedKeys); //用户使用ccKey属性的话，必需显示的指定ccClassKey
  ...
  connectedModules.forEach(function (m) {
    connectedComputed[m] = (0, _makeCuRefObContainer["default"])(ref, m, false);
  });
  var moduleComputed = (0, _makeCuRefObContainer["default"])(ref, module); // 所有实例都自动连接上了global模块，这里可直接取connectedComputed已做好的结果

  var globalComputed = connectedComputed[_constant.MODULE_GLOBAL];
  ...
  var moduleState = module === _constant.MODULE_GLOBAL ? globalState : (0, _makeObState["default"])(ref, mstate, module, true); // declare cc state series api
  ...
  var setState = function setState(p1, p2, p3, p4, p5) {
    ...
    if (p1Type === 'string') {
      ...
      _setState(stateModule, newState, _constant.SET_STATE, p2, p3, p4);
    } else {
  ...
  var forceUpdate = function forceUpdate(reactCallback, renderKey, delay) {
    _setState(stateModule, ref.unProxyState, _constant.FORCE_UPDATE, reactCallback, renderKey, delay);
  };
  ...
  var ctx = {
    ...
    __$$renderStatus: _privConstant.UNSTART,
    __$$curWaKeys: {},
    __$$compareWaKeys: {},
    ...
    // state
    state: (0, _makeObState["default"])(ref, mergedState, stateModule, true),
    ...
    var doSync = function doSync(e, val, rkey, delay, type) {
      if (typeof e === 'string') {
        ...
        if (isValueNotNull(val) && (valType === 'object' || valType === 'function')) {
          ...
          return _sync3["default"].bind(null, (_sync$bind = {}, _sync$bind[_constant.CCSYNC_KEY] = e, _sync$bind.type = type, _sync$bind.val = val, _sync$bind.delay = delay, _sync$bind.rkey = rkey, _sync$bind), ref);
        } else {
          ...
          if (!boundFn) {
            ...
            boundFn = cachedBoundFns[key] = _sync3["default"].bind(null, (_sync$bind2 = {}, _sync$bind2[_constant.CCSYNC_KEY] = e, _sync$bind2.type = type, _sync$bind2.val = val, _sync$bind2.delay = delay, _sync$bind2.rkey = rkey, _sync$bind2), ref);
          }
  ...
  if (liteLevel > 4) {
    ...
    ctx.computed = (0, _getDefineComputedHandler["default"])(ctx); // TODO 优化 effect入参格式：effect(cb, depKeysOrOpt)
  ...
  allModules.forEach(function (m) {
    ...
    if (m === module) {
      ...
      if (module === _constant.MODULE_GLOBAL) connectedReducer[_constant.MODULE_GLOBAL] = moduleReducer;
    } else {
    ...
    if (connectDesc) {
      ...
      if (connectDesc === '-') {
        ...
        if (m === _constant.MODULE_GLOBAL) _moduleState = ctx.globalState;else _moduleState = (0, _makeObState["default"])(ref, _moduleState, m);
