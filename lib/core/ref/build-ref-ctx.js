...
var getWatchedKeys = function getWatchedKeys(ctx) {
  if (ctx.watchedKeys === '-') {
    if (ctx.__$$renderStatus === START) return okeys(ctx.__$$compareWaKeys);else return okeys(ctx.__$$curWaKeys);
...
var getConnectWatchedKeys = function getConnectWatchedKeys(ctx, module) {
  ...
  var getModuleWaKeys = function getModuleWaKeys(m) {
    if (ctx.__$$renderStatus === START) return okeys(ctx.__$$compareConnWaKeys[m]);else return okeys(ctx.__$$curConnWaKeys[m]);
  };
    ...
    if (connectDesc) {
      ...
      if (connectDesc === '-') {
        ...
        mConnectedState = (0, _makeObState["default"])(ref, mConnectedState, m);
          ...
          var waKeys = connectDesc === '*' ? moduleName_stateKeys_[m] : connectDesc;
