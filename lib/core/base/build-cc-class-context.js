...
function _default(ccClassKey, moduleName, originalSharedStateKeys, sharedStateKeys, stateToPropMapping, connectedModuleNames, forCcFragment) {
  ...
  if (forCcFragment === true) {
    ...
    if (ccClassContext === undefined) {
      ccClassContext = _util["default"].makeCcClassContext(moduleName, ccClassKey, sharedStateKeys, originalSharedStateKeys);
    }
  } else {
