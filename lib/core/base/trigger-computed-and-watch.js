...
function _default(ref) {
  ...
  if (hasComputedFn) {
    (0, _computeValueForRef["default"])(ctx, refModule, refState, refState, true);
    connectedModules.forEach(function (m) {
      var mState = getState(m);
      (0, _computeValueForRef["default"])(ctx, m, mState, mState, true);
    });
  }

  if (hasWatchFn) {
    (0, _watchKeyForRef["default"])(ctx, refModule, refState, refState, true);
    connectedModules.forEach(function (m) {
      var mState = getState(m);
      (0, _watchKeyForRef["default"])(ctx, m, mState, mState, true);
    });
  }
}