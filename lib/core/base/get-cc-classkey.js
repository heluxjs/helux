...
function _default(allowNamingDispatcher, module, connect, prefix, featureStr, classKey) {
  if (classKey === void 0) {
    classKey = '';
  }
  ...
  if (!classKey) {
    // 未指定所属模块，也未连接到其他模块
    if (module === _constant.MODULE_DEFAULT && isObjectNull(connect)) {
      return prefix + "0";
    }
  ...
  if (!allowNamingDispatcher) {
    if (classKey.toLowerCase() === _constant.CC_DISPATCHER.toLowerCase()) {
      ...
      throw new Error(_constant.CC_DISPATCHER + " is cc built-in ccClassKey name.");
    }
  }

  var clsCtx = ccClassKey_ccClassContext_[classKey];
