...
function _default(cate, confMeta, item, handler, depKeys, compare, immediate) {
  ...
  if (itype === 'string') {
    ...
    // retKey
    if ((0, _util.isPlainJsonObject)(handler)) _descObj = (_descObj2 = {}, _descObj2[item] = handler, _descObj2);else _descObj = (_descObj3 = {}, _descObj3[item] = {
      fn: handler,
      depKeys: depKeys,
      compare: compare,
      immediate: immediate
    }, _descObj3);
  } else if ((0, _util.isPlainJsonObject)(item)) {
    _descObj = item;
  ...
  if (!_descObj) {
    (0, _util.justWarning)(cate + " " + confMeta.type + " param type error");
    return;
  }
...
function _parseDescObj(cate, confMeta, descObj) {
  ...
  (0, _util.okeys)(descObj).forEach(function (retKey) {
    ...
    if ((0, _util.isPlainJsonObject)(targetItem)) {
