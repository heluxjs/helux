...
var _executeAsyncCuInfo = _interopRequireDefault(require("../computed/execute-async-cu-info"));
...
var noCommit = function noCommit(tip, asIs) {
  return (0, _util.justWarning)(tip + " call commit or commitCu as it is " + asIs);
...
function executeDepFns(ref, stateModule, refModule, oldState, finder, committedState, initNewState, initDeltaCommittedState, callInfo, isFirstCall, fnType, sourceType, computedContainer, mergeToDelta) {
  ...
  var _loop = function _loop() {
    ...
    pickedFns.forEach(function (_ref) {
      ...
      var fnCtx = {
        ...
        setInitialVal: function setInitialVal() {
          beforeMountFlag && (0, _util.justWarning)("non async " + keyInfo + " call setInitialVal is unnecessary");
        }
      ...
      if (needCollectDep) {
        ...
        fnCtx.cuVal = (0, _makeCuRefObContainer.getSimpleObContainer)(retKey, sourceType, fnType, stateModule, refCtx, collectedCuRetKeys, referInfo);
      }

      if (fnType === _constant.FN_CU) {
        var isCuFnAsync = (0, _util.isAsyncFn)(fn);
        ...
        if (isLazy) {
          computedContainer[retKey] = (0, _util.makeCuPackedValue)(isLazy, null, true, fn, initNewState, oldState, fnCtx);
        } else {
          var newStateArg = initNewState,
              oldStateArg = oldState; // 首次计算时，new 和 old是同一个对象，方便用于收集depKeys

          if (needCollectDep) {
            newStateArg = oldStateArg = (0, _makeCuObState["default"])(initNewState, collectedDepKeys);
          ...
          if (isCuFnAsync || referInfo.hasAsyncCuRefer) {
            ...
            fnCtx.setInitialVal = _util.noop;
          ...
          computedContainer[retKey] = (0, _util.makeCuPackedValue)(false, computedRet);
      ...
      if (committedCuRet) {
        ...
        (0, _util.okeys)(committedCuRet).forEach(function (cuRetKey) {
          ...
          if (!fnDesc) (0, _util.justWarning)("commitCu:" + tip + " commit [" + cuRetKey + "], it is not defined"); // 由committedCu提交的值，可以统一当作非lazy值set回去，方便取的时候直接取
          else {
              ...
              if (fnDesc.isStatic) {
                ...
                if (RSList.includes(cuRetKey)) {
                  ...
                  (0, _util.justWarning)("commitCu:" + tip + " change [" + cuRetKey + "], [" + retKey + "] referred [" + cuRetKey + "]");
                } else {
                  computedContainer[cuRetKey] = (0, _util.makeCuPackedValue)(false, committedCuRet[cuRetKey]);
                }
              } else {
                (0, _util.justWarning)("commitCu:" + tip + " change [" + cuRetKey + "], it must have zero dep keys");
              }
            }
        });
        clearCu();
    ...
    if (curStateForComputeFn) {
      ...
      var ensureCommittedState = function ensureCommittedState(fnCommittedState) {
        ...
        var _extractStateByKeys = (0, _extractStateByKeys2["default"])(fnCommittedState, stateKeys, true),
    ...
    if (whileCount > 2) {
      (0, _util.justWarning)('fnCtx.commit may goes endless loop, please check your code'); // 清空，确保不再触发while循环
  ...
  (0, _executeAsyncCuInfo["default"])(nextTickCuInfo);
  return {
    hasDelta: hasDelta,
