...
function setStateKeyRetKeysMap(refCtx, sourceType, stateModule, retKey, keys, isKeysDep) {
  ...
  if (sourceType === _constant.CATE_REF) {
    modDep = (0, _util.safeGet)(refCtx.computedDep, refCtx.module);
  } else {
    modDep = (0, _util.safeGet)(_computedMap["default"]._computedDep, stateModule);
  }
  ...
  var _loop = function _loop() {
    ...
    pickedFns.forEach(function (_ref) {
      ...
      if (fnType === 'computed') {
        ...
        // watch
        fnCtx.cuVal = (0, _makeCuRefObContainer["default"])(ref, stateModule, true, true, collectedRetKeys);
          ...
          if (retKey_fn_) {
            (0, _util.okeys)(committedCu).forEach(function (retKey) {
              if (!retKey_fn_[retKey]) (0, _util.justWarning)("fnCtx.commitCu commit an invalid retKey[" + retKey + "] for moduleComputed"); // 由committedCu提交的值，可以统一当作非lazy值set回去，方便取的时候直接取
                  ...
                  computedContainer[retKey] = (0, _util.makeCuObValue)(false, committedCu[retKey]); // 在watch里读取了cuVal里的结果更新了cu里另外retKey的结果，就要有依赖产生
