...
function configStoreState(storeState) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    (0, _initModuleState["default"])(_moduleName, moduleState);
    var sharedKey_globalKey_ = sharedToGlobalMapping[_moduleName];

    if (sharedKey_globalKey_) {
      (0, _handleModuleSharedToGlobalMapping["default"])(_moduleName, sharedKey_globalKey_);
    }
  }
...
function configRootReducer(rootReducer) {
  ...
  for (var i = 0; i < len; i++) {
    (0, _initModuleReducer["default"])(moduleNames[i], rootReducer[moduleName]);
  }
}
...
function executeRootInit(init) {
  ...
  moduleNames.forEach(function (moduleName) {
    ...
    if (initFn) {
      initFn((0, _makeSetStateHandler["default"])(moduleName));
    }
  });
}
