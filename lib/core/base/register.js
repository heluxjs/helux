...
var _ccContext$store = _ccContext["default"].store,
    ...
    moduleName_watchedKeys_ = _ccContext["default"].moduleName_watchedKeys_,
...
function mapModuleAndWatchedStateKeys(moduleName, partialWatchedKeys) {
  var watchedKeysOfModule = _util["default"].safeGetArrayFromObject(moduleName_watchedKeys_, moduleName);
...
function mapCcClassKeyAndCcClassContext(ccClassKey, moduleName, originalWatchedKeys, watchedKeys, connectSpec) {
  ...
  (0, _buildCcClassContext["default"])(ccClassKey, moduleName, originalWatchedKeys, watchedKeys, stateToPropMapping, connectedModuleNames);
}
    ...
    return function (ReactClass) {
      ...
      function (_ToBeExtendedClass) {
        ...
        _proto.__$$mapCcToInstance = function __$$mapCcToInstance(isSingle, ccClassKey, originalCcKey, ccKey, ccUniqueKey, isCcUniqueKeyAutoGenerated, storedKeys, ccOption, ccClassContext, currentModule, currentReducerModule, watchedKeys, connect) {
          ...
          var isControlledByConcent = watchedKeys.length > 0 || _util["default"].isObjectNotNull(connect);
          ...
          var _verifyKeys2 = verifyKeys(watchedKeys, storedKeys),
              duplicate = _verifyKeys2.duplicate,
              notArray = _verifyKeys2.notArray,
              keyElementNotString = _verifyKeys2.keyElementNotString;

          if (notArray) {
            throw me(_constant.ERR.CC_ARG_KEYS_NOT_AN_ARRAY, vbi("ccClassKey:" + ccClassKey + " ccKey:" + ccKey));
          }

          if (keyElementNotString) {
            throw me(_constant.ERR.CC_ARG_KEYS_INCLUDE_NON_STRING_ELEMENT, vbi("ccClassKey:" + ccClassKey + " ccKey:" + ccKey));
          }

          if (duplicate) {
            throw me(_constant.ERR.CC_ARG_STORED_KEYS_DUPLICATE_WITH_WATCHED_KEYS, vbi("ccClassKey:" + ccClassKey + " ccKey:" + ccKey + " watchedKeys:" + watchedKeys + " storedKeys:" + storedKeys));
          }

          if (storedKeys.length > 0 && isCcUniqueKeyAutoGenerated) {
            throw me(_constant.ERR.CC_CLASS_INSTANCE_NO_CC_KEY_SPECIFIED_WHEN_USE_STORED_KEYS, vbi("ccClassKey:" + ccClassKey));
          }

          var connectedState = ccClassContext.connectedState || {};
          var globalState = getState(_constant.MODULE_GLOBAL);
          var moduleState = getState(_curStateModule);
          this.cc = {
