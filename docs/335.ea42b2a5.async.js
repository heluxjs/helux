(self.webpackChunkhelux_docs = self.webpackChunkhelux_docs || []).push([
  [335],
  {
    41226: function (j, h, r) {
      'use strict';
      r.d(h, {
        i: function () {
          return W;
        },
      });
      var e = r(79664),
        I = r.n(e),
        v = r(70125),
        O = r.n(v),
        w = r(70079),
        z = r(77937),
        Q = r.n(z),
        T = r(41176),
        E = r.n(T),
        N = r(44114),
        y = r.n(N),
        D = r(65523),
        x = r.n(D),
        f = r(7122),
        M = r.n(f),
        i = r(79862),
        p = r.n(i),
        S = E()(function B() {
          Q()(this, B);
        }),
        b = (function (B) {
          x()(C, B);
          var R = M()(C);
          function C(A) {
            var o;
            return Q()(this, C), (o = R.call(this)), p()(y()(o), 'el', void 0), (o.el = A), o;
          }
          return (
            E()(
              C,
              [
                {
                  key: 'top',
                  get: function () {
                    return this.el.getBoundingClientRect().top;
                  },
                },
                {
                  key: 'outerHeight',
                  get: function () {
                    return this.el.getBoundingClientRect().height;
                  },
                },
                {
                  key: 'scrollTop',
                  get: function () {
                    return this.el.scrollTop;
                  },
                },
                {
                  key: 'scrollHeight',
                  get: function () {
                    return this.el.scrollHeight;
                  },
                },
                {
                  key: 'isScrolledToBottom',
                  value: function () {
                    return this.scrollTop + this.outerHeight >= this.scrollHeight;
                  },
                },
                {
                  key: 'registerScrollEvent',
                  value: function (o) {
                    this.el.addEventListener('scroll', o);
                  },
                },
                {
                  key: 'unregisterScrollEvent',
                  value: function (o) {
                    this.el.removeEventListener('scroll', o);
                  },
                },
              ],
              [
                {
                  key: 'create',
                  value: function (o) {
                    var g = document.querySelector(o);
                    if (!g) throw new Error('element is not found.');
                    return new C(g);
                  },
                },
              ],
            ),
            C
          );
        })(S),
        P = (function (B) {
          x()(C, B);
          var R = M()(C);
          function C() {
            return Q()(this, C), R.apply(this, arguments);
          }
          return (
            E()(
              C,
              [
                {
                  key: 'outerHeight',
                  get: function () {
                    return window.innerHeight;
                  },
                },
                {
                  key: 'scrollTop',
                  get: function () {
                    return document.documentElement.scrollTop;
                  },
                },
                {
                  key: 'scrollHeight',
                  get: function () {
                    return document.documentElement.scrollHeight;
                  },
                },
                {
                  key: 'isScrolledToBottom',
                  value: function () {
                    return this.scrollTop + this.outerHeight >= this.scrollHeight;
                  },
                },
                {
                  key: 'registerScrollEvent',
                  value: function (o) {
                    document.addEventListener('scroll', o);
                  },
                },
                {
                  key: 'unregisterScrollEvent',
                  value: function (o) {
                    document.removeEventListener('scroll', o);
                  },
                },
              ],
              [
                {
                  key: 'create',
                  value: function () {
                    return new C();
                  },
                },
              ],
            ),
            C
          );
        })(S),
        k = (function () {
          function B() {
            Q()(this, B);
          }
          return (
            E()(B, null, [
              {
                key: 'create',
                value: function (C) {
                  return C ? b.create(C) : P.create();
                },
              },
            ]),
            B
          );
        })(),
        U = function (R) {
          var C = R.sectionRefs,
            A = R.rootSelector,
            o = R.offset,
            g = o === void 0 ? 0 : o,
            d = (0, w.useRef)(null);
          (0, w.useEffect)(
            function () {
              d.current = k.create(A);
            },
            [A],
          );
          var V = (0, w.useCallback)(
              function () {
                return d.current ? d.current.isScrolledToBottom() : !1;
              },
              [d],
            ),
            J = (0, w.useCallback)(
              function (Y) {
                if (!d.current) return !1;
                var u = d.current.scrollTop,
                  c = u + d.current.outerHeight,
                  l = Y.getBoundingClientRect(),
                  F = d.current instanceof b ? u + l.top - d.current.top + g : u + l.top + g,
                  Z = F + l.height;
                return [F < c, Z > u].every(function (_) {
                  return _;
                });
              },
              [d, g],
            ),
            X = (0, w.useCallback)(
              function () {
                return C.map(function (Y) {
                  return Y.current ? J(Y.current) : !1;
                });
              },
              [J, C],
            ),
            K = (0, w.useState)([]),
            t = O()(K, 2),
            n = t[0],
            a = t[1],
            G = (0, w.useMemo)(
              function () {
                return n.findIndex(function (Y) {
                  return Y;
                });
              },
              [n],
            ),
            H = (0, w.useCallback)(
              function () {
                var Y = V()
                  ? [].concat(
                      I()(
                        new Array(C.length - 1).fill(!1).map(function (u) {
                          return u;
                        }),
                      ),
                      [!0],
                    )
                  : X();
                a(Y);
              },
              [X, V, C],
            );
          return (
            (0, w.useEffect)(
              function () {
                return (
                  H(),
                  d.current && d.current.registerScrollEvent(H),
                  function () {
                    d.current && d.current.unregisterScrollEvent(H);
                  }
                );
              },
              [H],
            ),
            { elementsStatusInViewport: n, currentElementIndexInViewport: G }
          );
        },
        W = function (R) {
          var C = R.children,
            A = R.sectionRefs,
            o = R.rootSelector,
            g = R.offset,
            d = U({ sectionRefs: A, rootSelector: o, offset: g }),
            V = d.elementsStatusInViewport,
            J = d.currentElementIndexInViewport;
          return C({ elementsStatusInViewport: V, currentElementIndexInViewport: J });
        };
    },
    38527: function (j, h, r) {
      'use strict';
      r.r(h),
        r.d(h, {
          default: function () {
            return A;
          },
        });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (o, g, d) => (g in o ? I(o, g, { enumerable: !0, configurable: !0, writable: !0, value: d }) : (o[g] = d)),
        Q = (o, g) => {
          for (var d in g || (g = {})) O.call(g, d) && z(o, d, g[d]);
          if (v) for (var d of v(g)) w.call(g, d) && z(o, d, g[d]);
          return o;
        };
      const T = (o) =>
        e.createElement(
          'svg',
          Q({ viewBox: '64 64 896 896' }, o),
          e.createElement('path', {
            d: 'M120 230h496c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8zm0 424h496c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8zm784 140H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8zm0-424H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8z',
          }),
        );
      var E =
          'data:image/svg+xml;base64,',
        N = r(22484),
        y = r(80904),
        D = r(44044),
        x = r(66803),
        f = r(15346),
        M = r(72686),
        i = r(34395),
        p = r(88500),
        S = r(33731),
        b = r(87482);
      function P(o, g) {
        return R(o) || B(o, g) || U(o, g) || k();
      }
      function k() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function U(o, g) {
        if (o) {
          if (typeof o == 'string') return W(o, g);
          var d = Object.prototype.toString.call(o).slice(8, -1);
          if ((d === 'Object' && o.constructor && (d = o.constructor.name), d === 'Map' || d === 'Set')) return Array.from(o);
          if (d === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(d)) return W(o, g);
        }
      }
      function W(o, g) {
        (g == null || g > o.length) && (g = o.length);
        for (var d = 0, V = new Array(g); d < g; d++) V[d] = o[d];
        return V;
      }
      function B(o, g) {
        var d = o == null ? null : (typeof Symbol != 'undefined' && o[Symbol.iterator]) || o['@@iterator'];
        if (d != null) {
          var V,
            J,
            X,
            K,
            t = [],
            n = !0,
            a = !1;
          try {
            if (((X = (d = d.call(o)).next), g === 0)) {
              if (Object(d) !== d) return;
              n = !1;
            } else for (; !(n = (V = X.call(d)).done) && (t.push(V.value), t.length !== g); n = !0);
          } catch (G) {
            (a = !0), (J = G);
          } finally {
            try {
              if (!n && d.return != null && ((K = d.return()), Object(K) !== K)) return;
            } finally {
              if (a) throw J;
            }
          }
          return t;
        }
      }
      function R(o) {
        if (Array.isArray(o)) return o;
      }
      var C = function () {
          var g = (0, y.YB)(),
            d = (0, y.pC)(),
            V = (0, y.tx)(),
            J = (0, y.TH)(),
            X = J.hash,
            K = J.pathname,
            t = (0, y.WF)(),
            n = t.loading,
            a = t.hostname,
            G = (0, e.useState)(!1),
            H = P(G, 2),
            Y = H[0],
            u = H[1],
            c = (0, y.eL)(),
            l = c.frontmatter,
            F = l.sidebar !== !1 && (V == null ? void 0 : V.length) > 0;
          return (
            (0, e.useEffect)(
              function () {
                var Z = X.replace('#', '');
                Z
                  && setTimeout(function () {
                    var _ = document.getElementById(decodeURIComponent(Z));
                    _ && (0, N.Z)(_.offsetTop - 80, { maxDuration: 300 });
                  }, 1);
              },
              [n, X],
            ),
            e.createElement(
              'div',
              {
                className: 'dumi-default-doc-layout',
                'data-mobile-sidebar-active': Y || void 0,
                onClick: function () {
                  return u(!1);
                },
              },
              e.createElement(
                y.ql,
                null,
                e.createElement('html', { lang: g.locale.replace(/-.+$/, '') }),
                l.title && e.createElement('title', null, l.title),
                l.title && e.createElement('meta', { property: 'og:title', content: l.title }),
                l.description && e.createElement('meta', { name: 'description', content: l.description }),
                l.description && e.createElement('meta', { property: 'og:description', content: l.description }),
                l.keywords && e.createElement('meta', { name: 'keywords', content: l.keywords.join(',') }),
                l.keywords
                  && l.keywords.map(function (Z) {
                    return e.createElement('meta', { key: Z, property: 'article:tag', content: Z });
                  }),
                a && e.createElement('link', { rel: 'canonical', href: a + K }),
              ),
              e.createElement(i.Z, null),
              e.createElement(p.Z, null),
              e.createElement(f.Z, null),
              F
                && e.createElement(
                  'div',
                  { className: 'dumi-default-doc-layout-mobile-bar' },
                  e.createElement(
                    'button',
                    {
                      type: 'button',
                      className: 'dumi-default-sidebar-btn',
                      onClick: function (_) {
                        _.stopPropagation(),
                          u(function ($) {
                            return !$;
                          });
                      },
                    },
                    e.createElement(T, null),
                    g.formatMessage({ id: 'layout.sidebar.btn' }),
                  ),
                ),
              e.createElement(
                'main',
                null,
                F && e.createElement(S.Z, null),
                e.createElement(D.Z, null, e.createElement('article', null, d), e.createElement(x.Z, null), e.createElement(M.Z, null)),
                l.toc === 'content'
                  && e.createElement(
                    'div',
                    { className: 'dumi-default-doc-layout-toc-wrapper' },
                    e.createElement('h4', null, 'TABLE OF CONTENTS'),
                    e.createElement(b.Z, null),
                  ),
              ),
            )
          );
        },
        A = C;
    },
    32800: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return f;
        },
      });
      var e = r(80904),
        I = r(70079);
      function v(M, i) {
        return T(M) || Q(M, i) || w(M, i) || O();
      }
      function O() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function w(M, i) {
        if (M) {
          if (typeof M == 'string') return z(M, i);
          var p = Object.prototype.toString.call(M).slice(8, -1);
          if ((p === 'Object' && M.constructor && (p = M.constructor.name), p === 'Map' || p === 'Set')) return Array.from(M);
          if (p === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(p)) return z(M, i);
        }
      }
      function z(M, i) {
        (i == null || i > M.length) && (i = M.length);
        for (var p = 0, S = new Array(i); p < i; p++) S[p] = M[p];
        return S;
      }
      function Q(M, i) {
        var p = M == null ? null : (typeof Symbol != 'undefined' && M[Symbol.iterator]) || M['@@iterator'];
        if (p != null) {
          var S,
            b,
            P,
            k,
            U = [],
            W = !0,
            B = !1;
          try {
            if (((P = (p = p.call(M)).next), i === 0)) {
              if (Object(p) !== p) return;
              W = !1;
            } else for (; !(W = (S = P.call(p)).done) && (U.push(S.value), U.length !== i); W = !0);
          } catch (R) {
            (B = !0), (b = R);
          } finally {
            try {
              if (!W && p.return != null && ((k = p.return()), Object(k) !== k)) return;
            } finally {
              if (B) throw b;
            }
          }
          return U;
        }
      }
      function T(M) {
        if (Array.isArray(M)) return M;
      }
      var E = function () {
          return I.createElement(
            'svg',
            { viewBox: '0 0 16 16' },
            I.createElement('path', {
              d: 'M8.218 1.455c3.527.109 6.327 3.018 6.327 6.545 0 3.6-2.945 6.545-6.545 6.545a6.562 6.562 0 0 1-6.036-4h.218c3.6 0 6.545-2.945 6.545-6.545 0-.91-.182-1.745-.509-2.545m0-1.455c-.473 0-.909.218-1.2.618-.29.4-.327.946-.145 1.382.254.655.4 1.31.4 2 0 2.8-2.291 5.09-5.091 5.09h-.218c-.473 0-.91.22-1.2.62-.291.4-.328.945-.146 1.38C1.891 14.074 4.764 16 8 16c4.4 0 8-3.6 8-8a7.972 7.972 0 0 0-7.745-8h-.037Z',
            }),
          );
        },
        N = function () {
          return I.createElement(
            'svg',
            { viewBox: '0 0 16 16' },
            I.createElement('path', {
              d: 'M8 13a1 1 0 0 1 1 1v1a1 1 0 1 1-2 0v-1a1 1 0 0 1 1-1ZM8 3a1 1 0 0 1-1-1V1a1 1 0 1 1 2 0v1a1 1 0 0 1-1 1Zm7 4a1 1 0 1 1 0 2h-1a1 1 0 1 1 0-2h1ZM3 8a1 1 0 0 1-1 1H1a1 1 0 1 1 0-2h1a1 1 0 0 1 1 1Zm9.95 3.536.707.707a1 1 0 0 1-1.414 1.414l-.707-.707a1 1 0 0 1 1.414-1.414Zm-9.9-7.072-.707-.707a1 1 0 0 1 1.414-1.414l.707.707A1 1 0 0 1 3.05 4.464Zm9.9 0a1 1 0 0 1-1.414-1.414l.707-.707a1 1 0 0 1 1.414 1.414l-.707.707Zm-9.9 7.072a1 1 0 0 1 1.414 1.414l-.707.707a1 1 0 0 1-1.414-1.414l.707-.707ZM8 4a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm0 6.5a2.5 2.5 0 1 1 0-5 2.5 2.5 0 0 1 0 5Z',
            }),
          );
        },
        y = function () {
          return I.createElement(
            'svg',
            { viewBox: '0 0 16 16' },
            I.createElement('path', {
              d: 'M14.595 8a6.595 6.595 0 1 1-13.19 0 6.595 6.595 0 0 1 13.19 0ZM8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0Zm0 2.014v11.972A5.986 5.986 0 0 0 8 2.014Z',
            }),
          );
        },
        D = { light: N, dark: E, auto: y },
        x = function () {
          var i = (0, e.WF)(),
            p = i.themeConfig.prefersColor.default,
            S = (0, e.YB)(),
            b = (0, e.OI)(),
            P = v(b, 3),
            k = P[1],
            U = k === void 0 ? p : k,
            W = P[2],
            B = D[U];
          return I.createElement(
            'span',
            {
              className: 'dumi-default-color-switch',
              'data-dumi-tooltip': S.formatMessage({ id: 'header.color.mode.'.concat(U) }),
              'data-dumi-tooltip-bottom': !0,
            },
            B && I.createElement(B, null),
            I.createElement(
              'select',
              {
                onChange: function (C) {
                  return W(C.target.value);
                },
                value: U,
                onClick: function (C) {
                  return C.stopPropagation();
                },
              },
              ['light', 'dark', 'auto'].map(function (R) {
                return I.createElement('option', { value: R, key: R }, S.formatMessage({ id: 'header.color.mode.'.concat(R) }));
              }),
            ),
          );
        },
        f = x;
    },
    29684: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return O;
        },
      });
      var e = r(80904),
        I = r(70079),
        v = function (z) {
          var Q = (0, e.tx)(),
            T = (0, e.WF)(),
            E = T.themeConfig,
            N = (0, e.eL)(),
            y = N.frontmatter;
          return I.createElement(
            'div',
            {
              className: 'dumi-default-content',
              'data-no-sidebar': !Q || y.sidebar === !1 || void 0,
              'data-no-footer': E.footer === !1 || void 0,
            },
            z.children,
          );
        },
        O = v;
    },
    35750: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return K;
        },
      });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (t, n, a) => (n in t ? I(t, n, { enumerable: !0, configurable: !0, writable: !0, value: a }) : (t[n] = a)),
        Q = (t, n) => {
          for (var a in n || (n = {})) O.call(n, a) && z(t, a, n[a]);
          if (v) for (var a of v(n)) w.call(n, a) && z(t, a, n[a]);
          return t;
        };
      const T = (t) =>
        e.createElement(
          'svg',
          Q({ viewBox: '64 64 896 896' }, t),
          e.createElement('path', {
            d: 'M872 474H286.9l350.2-304c5.6-4.9 2.2-14-5.2-14h-88.5c-3.9 0-7.6 1.4-10.5 3.9L155 487.8a31.96 31.96 0 0 0 0 48.3L535.1 866c1.5 1.3 3.3 2 5.2 2h91.5c7.4 0 10.8-9.2 5.2-14L286.9 550H872c4.4 0 8-3.6 8-8v-60c0-4.4-3.6-8-8-8z',
          }),
        );
      var E =
          'data:image/svg+xml;base64,',
        N = Object.defineProperty,
        y = Object.getOwnPropertySymbols,
        D = Object.prototype.hasOwnProperty,
        x = Object.prototype.propertyIsEnumerable,
        f = (t, n, a) => (n in t ? N(t, n, { enumerable: !0, configurable: !0, writable: !0, value: a }) : (t[n] = a)),
        M = (t, n) => {
          for (var a in n || (n = {})) D.call(n, a) && f(t, a, n[a]);
          if (y) for (var a of y(n)) x.call(n, a) && f(t, a, n[a]);
          return t;
        };
      const i = (t) =>
        e.createElement(
          'svg',
          M({ viewBox: '64 64 896 896' }, t),
          e.createElement('path', {
            d: 'M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm0 820c-205.4 0-372-166.6-372-372s166.6-372 372-372 372 166.6 372 372-166.6 372-372 372z',
          }),
          e.createElement('path', {
            d: 'M686.7 638.6 544.1 535.5V288c0-4.4-3.6-8-8-8H488c-4.4 0-8 3.6-8 8v275.4c0 2.6 1.2 5 3.3 6.5l165.4 120.6c3.6 2.6 8.6 1.8 11.2-1.7l28.6-39c2.6-3.7 1.8-8.7-1.8-11.2z',
          }),
        );
      var p =
          'data:image/svg+xml;base64,',
        S = Object.defineProperty,
        b = Object.getOwnPropertySymbols,
        P = Object.prototype.hasOwnProperty,
        k = Object.prototype.propertyIsEnumerable,
        U = (t, n, a) => (n in t ? S(t, n, { enumerable: !0, configurable: !0, writable: !0, value: a }) : (t[n] = a)),
        W = (t, n) => {
          for (var a in n || (n = {})) P.call(n, a) && U(t, a, n[a]);
          if (b) for (var a of b(n)) k.call(n, a) && U(t, a, n[a]);
          return t;
        };
      const B = (t) =>
        e.createElement(
          'svg',
          W({ viewBox: '64 64 896 896' }, t),
          e.createElement('path', {
            d: 'M257.7 752c2 0 4-.2 6-.5L431.9 722c2-.4 3.9-1.3 5.3-2.8l423.9-423.9a9.96 9.96 0 0 0 0-14.1L694.9 114.9c-1.9-1.9-4.4-2.9-7.1-2.9s-5.2 1-7.1 2.9L256.8 538.8c-1.5 1.5-2.4 3.3-2.8 5.3l-29.5 168.2a33.5 33.5 0 0 0 9.4 29.8c6.6 6.4 14.9 9.9 23.8 9.9zm67.4-174.4L687.8 215l73.3 73.3-362.7 362.6-88.9 15.7 15.6-89zM880 836H144c-17.7 0-32 14.3-32 32v36c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-36c0-17.7-14.3-32-32-32z',
          }),
        );
      var R =
          'data:image/svg+xml;base64,',
        C = r(80904);
      function A(t, n) {
        return J(t) || V(t, n) || g(t, n) || o();
      }
      function o() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function g(t, n) {
        if (t) {
          if (typeof t == 'string') return d(t, n);
          var a = Object.prototype.toString.call(t).slice(8, -1);
          if ((a === 'Object' && t.constructor && (a = t.constructor.name), a === 'Map' || a === 'Set')) return Array.from(t);
          if (a === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(a)) return d(t, n);
        }
      }
      function d(t, n) {
        (n == null || n > t.length) && (n = t.length);
        for (var a = 0, G = new Array(n); a < n; a++) G[a] = t[a];
        return G;
      }
      function V(t, n) {
        var a = t == null ? null : (typeof Symbol != 'undefined' && t[Symbol.iterator]) || t['@@iterator'];
        if (a != null) {
          var G,
            H,
            Y,
            u,
            c = [],
            l = !0,
            F = !1;
          try {
            if (((Y = (a = a.call(t)).next), n === 0)) {
              if (Object(a) !== a) return;
              l = !1;
            } else for (; !(l = (G = Y.call(a)).done) && (c.push(G.value), c.length !== n); l = !0);
          } catch (Z) {
            (F = !0), (H = Z);
          } finally {
            try {
              if (!l && a.return != null && ((u = a.return()), Object(u) !== u)) return;
            } finally {
              if (F) throw H;
            }
          }
          return c;
        }
      }
      function J(t) {
        if (Array.isArray(t)) return t;
      }
      var X = function () {
          var n = (0, C.TH)(),
            a = n.pathname,
            G = (0, C.tx)(),
            H = (0, C.WF)(),
            Y = H.themeConfig,
            u = (0, C.eL)(),
            c = u.frontmatter,
            l = (0, C.YB)(),
            F = (0, e.useState)(void 0),
            Z = A(F, 2),
            _ = Z[0],
            $ = Z[1],
            te = (0, e.useState)(void 0),
            ee = A(te, 2),
            ne = ee[0],
            ce = ee[1],
            ae = (0, e.useState)(''),
            se = A(ae, 2),
            oe = se[0],
            le = se[1],
            ie = (0, e.useState)(''),
            Me = A(ie, 2),
            fe = Me[0],
            ge = Me[1],
            ue = Y.editLink && c.filename,
            q = Y.lastUpdated && c.lastUpdated;
          return (
            (0, e.useLayoutEffect)(
              function () {
                if (G) {
                  var re = G.reduce(function (s, m) {
                      return s.concat(m.children);
                    }, []),
                    L = re.findIndex(function (s) {
                      return s.link === a;
                    });
                  $(re[L - 1]), ce(re[L + 1]);
                }
              },
              [a, G],
            ),
            (0, e.useLayoutEffect)(
              function () {
                q
                  && (le(new Date(c.lastUpdated).toISOString()),
                  ge(new Intl.DateTimeFormat(void 0, { dateStyle: 'short', timeStyle: 'short' }).format(c.lastUpdated)));
              },
              [q],
            ),
            e.createElement(
              'footer',
              { className: 'dumi-default-content-footer' },
              e.createElement(
                'dl',
                null,
                q
                  && e.createElement(
                    'dd',
                    null,
                    e.createElement(i, null),
                    e.createElement(C._H, { id: 'content.footer.last.updated' }),
                    e.createElement('time', { dateTime: oe }, fe),
                  ),
                ue
                  && e.createElement(
                    'dd',
                    null,
                    e.createElement(
                      'a',
                      {
                        target: '_blank',
                        href: ''.concat(l.formatMessage({ id: '$internal.edit.link' }, { filename: c.filename })),
                        rel: 'noreferrer',
                      },
                      e.createElement(B, null),
                      e.createElement(C._H, { id: 'content.footer.actions.edit' }),
                    ),
                  ),
              ),
              e.createElement(
                'nav',
                null,
                _
                  && e.createElement(
                    C.rU,
                    { to: _.link, 'data-prev': !0 },
                    e.createElement(
                      'small',
                      null,
                      e.createElement(T, null),
                      e.createElement(C._H, { id: 'content.footer.actions.previous' }),
                    ),
                    _.title,
                  ),
                ne
                  && e.createElement(
                    C.rU,
                    { to: ne.link, 'data-next': !0 },
                    e.createElement('small', null, e.createElement(C._H, { id: 'content.footer.actions.next' }), e.createElement(T, null)),
                    ne.title,
                  ),
              ),
            )
          );
        },
        K = X;
    },
    71314: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return K;
        },
      });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (t, n, a) => (n in t ? I(t, n, { enumerable: !0, configurable: !0, writable: !0, value: a }) : (t[n] = a)),
        Q = (t, n) => {
          for (var a in n || (n = {})) O.call(n, a) && z(t, a, n[a]);
          if (v) for (var a of v(n)) w.call(n, a) && z(t, a, n[a]);
          return t;
        };
      const T = (t) =>
        e.createElement(
          'svg',
          Q({ fillRule: 'evenodd', viewBox: '64 64 896 896' }, t),
          e.createElement('path', {
            d: 'M799.86 166.31c.02 0 .04.02.08.06l57.69 57.7c.04.03.05.05.06.08a.12.12 0 0 1 0 .06c0 .03-.02.05-.06.09L569.93 512l287.7 287.7c.04.04.05.06.06.09a.12.12 0 0 1 0 .07c0 .02-.02.04-.06.08l-57.7 57.69c-.03.04-.05.05-.07.06a.12.12 0 0 1-.07 0c-.03 0-.05-.02-.09-.06L512 569.93l-287.7 287.7c-.04.04-.06.05-.09.06a.12.12 0 0 1-.07 0c-.02 0-.04-.02-.08-.06l-57.69-57.7a.169.169 0 0 1-.06-.07.12.12 0 0 1 0-.07c0-.03.02-.05.06-.09L454.07 512l-287.7-287.7a.199.199 0 0 1-.06-.09.12.12 0 0 1 0-.07c0-.02.02-.04.06-.08l57.7-57.69c.03-.04.05-.05.07-.06a.12.12 0 0 1 .07 0c.03 0 .05.02.09.06L512 454.07l287.7-287.7c.04-.04.06-.05.09-.06a.12.12 0 0 1 .07 0z',
          }),
        );
      var E =
          'data:image/svg+xml;base64,',
        N = Object.defineProperty,
        y = Object.getOwnPropertySymbols,
        D = Object.prototype.hasOwnProperty,
        x = Object.prototype.propertyIsEnumerable,
        f = (t, n, a) => (n in t ? N(t, n, { enumerable: !0, configurable: !0, writable: !0, value: a }) : (t[n] = a)),
        M = (t, n) => {
          for (var a in n || (n = {})) D.call(n, a) && f(t, a, n[a]);
          if (y) for (var a of y(n)) x.call(n, a) && f(t, a, n[a]);
          return t;
        };
      const i = (t) =>
        e.createElement(
          'svg',
          M({ viewBox: '64 64 896 896' }, t),
          e.createElement('path', {
            d: 'M904 160H120c-4.4 0-8 3.6-8 8v64c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-64c0-4.4-3.6-8-8-8zm0 624H120c-4.4 0-8 3.6-8 8v64c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-64c0-4.4-3.6-8-8-8zm0-312H120c-4.4 0-8 3.6-8 8v64c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-64c0-4.4-3.6-8-8-8z',
          }),
        );
      var p =
          'data:image/svg+xml;base64,',
        S = r(80904),
        b = r(70736),
        P = r(26415),
        k = r(17353),
        U = r(32776),
        W = r(10473),
        B = r(65989),
        R = r(22224),
        C = r(27618);
      function A(t, n) {
        return J(t) || V(t, n) || g(t, n) || o();
      }
      function o() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function g(t, n) {
        if (t) {
          if (typeof t == 'string') return d(t, n);
          var a = Object.prototype.toString.call(t).slice(8, -1);
          if ((a === 'Object' && t.constructor && (a = t.constructor.name), a === 'Map' || a === 'Set')) return Array.from(t);
          if (a === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(a)) return d(t, n);
        }
      }
      function d(t, n) {
        (n == null || n > t.length) && (n = t.length);
        for (var a = 0, G = new Array(n); a < n; a++) G[a] = t[a];
        return G;
      }
      function V(t, n) {
        var a = t == null ? null : (typeof Symbol != 'undefined' && t[Symbol.iterator]) || t['@@iterator'];
        if (a != null) {
          var G,
            H,
            Y,
            u,
            c = [],
            l = !0,
            F = !1;
          try {
            if (((Y = (a = a.call(t)).next), n === 0)) {
              if (Object(a) !== a) return;
              l = !1;
            } else for (; !(l = (G = Y.call(a)).done) && (c.push(G.value), c.length !== n); l = !0);
          } catch (Z) {
            (F = !0), (H = Z);
          } finally {
            try {
              if (!l && a.return != null && ((u = a.return()), Object(u) !== u)) return;
            } finally {
              if (F) throw H;
            }
          }
          return c;
        }
      }
      function J(t) {
        if (Array.isArray(t)) return t;
      }
      var X = function () {
          var n = (0, S.eL)(),
            a = n.frontmatter,
            G = (0, e.useState)(!1),
            H = A(G, 2),
            Y = H[0],
            u = H[1],
            c = (0, S.WF)(),
            l = c.themeConfig,
            F = (0, e.useMemo)(
              function () {
                return l.socialLinks
                  ? Object.keys(l.socialLinks)
                      .slice(0, 5)
                      .map(function (Z) {
                        return { icon: Z, link: l.socialLinks[Z] };
                      })
                  : [];
              },
              [l.socialLinks],
            );
          return e.createElement(
            'div',
            {
              className: 'dumi-default-header',
              'data-static': !!a.hero || void 0,
              'data-mobile-active': Y || void 0,
              onClick: function () {
                return u(!1);
              },
            },
            e.createElement(
              'div',
              { className: 'dumi-default-header-content' },
              e.createElement('section', { className: 'dumi-default-header-left' }, e.createElement(U.Z, null)),
              e.createElement(
                'section',
                { className: 'dumi-default-header-right' },
                e.createElement(W.Z, null),
                e.createElement(
                  'div',
                  { className: 'dumi-default-header-right-aside' },
                  e.createElement(R.ZP, null),
                  e.createElement(k.Z, null),
                  e.createElement(B.Z, null),
                  l.prefersColor.switch && e.createElement(b.Z, null),
                  F.map(function (Z) {
                    return e.createElement(C.Z, { icon: Z.icon, link: Z.link, key: Z.link });
                  }),
                  e.createElement(P.Z, null),
                ),
              ),
              e.createElement(
                'button',
                {
                  type: 'button',
                  className: 'dumi-default-header-menu-btn',
                  onClick: function (_) {
                    _.stopPropagation(),
                      u(function ($) {
                        return !$;
                      });
                  },
                },
                Y ? e.createElement(T, null) : e.createElement(i, null),
              ),
            ),
          );
        },
        K = X;
    },
    6391: function (j, h, r) {
      'use strict';
      var e = r(70079),
        I = function () {
          return e.createElement(e.Fragment, null);
        };
      h.Z = I;
    },
    70196: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return w;
        },
      });
      var e = r(80904),
        I = r(24417),
        v = r(70079),
        O = function () {
          var Q,
            T = (0, e.eL)(),
            E = T.frontmatter;
          return 'hero' in E
            ? v.createElement(
                'div',
                { className: 'dumi-default-hero' },
                E.hero.title && v.createElement(I.Z, null, E.hero.title),
                E.hero.description && v.createElement('p', { dangerouslySetInnerHTML: { __html: E.hero.description } }),
                !!(!((Q = E.hero.actions) === null || Q === void 0) && Q.length)
                  && v.createElement(
                    'div',
                    { className: 'dumi-default-hero-actions' },
                    E.hero.actions.map(function (N) {
                      var y = N.text,
                        D = N.link;
                      return /^(\w+:)\/\/|^(mailto|tel):/.test(D)
                        ? v.createElement('a', { href: D, target: '_blank', rel: 'noreferrer', key: y }, y)
                        : v.createElement(e.rU, { key: y, to: D }, y);
                    }),
                  ),
              )
            : null;
        },
        w = O;
    },
    27883: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return x;
        },
      });
      var e = r(30011),
        I = r(80904),
        v = r(70079);
      function O(f, M) {
        return E(f) || T(f, M) || z(f, M) || w();
      }
      function w() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function z(f, M) {
        if (f) {
          if (typeof f == 'string') return Q(f, M);
          var i = Object.prototype.toString.call(f).slice(8, -1);
          if ((i === 'Object' && f.constructor && (i = f.constructor.name), i === 'Map' || i === 'Set')) return Array.from(f);
          if (i === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(i)) return Q(f, M);
        }
      }
      function Q(f, M) {
        (M == null || M > f.length) && (M = f.length);
        for (var i = 0, p = new Array(M); i < M; i++) p[i] = f[i];
        return p;
      }
      function T(f, M) {
        var i = f == null ? null : (typeof Symbol != 'undefined' && f[Symbol.iterator]) || f['@@iterator'];
        if (i != null) {
          var p,
            S,
            b,
            P,
            k = [],
            U = !0,
            W = !1;
          try {
            if (((b = (i = i.call(f)).next), M === 0)) {
              if (Object(i) !== i) return;
              U = !1;
            } else for (; !(U = (p = b.call(i)).done) && (k.push(p.value), k.length !== M); U = !0);
          } catch (B) {
            (W = !0), (S = B);
          } finally {
            try {
              if (!U && i.return != null && ((P = i.return()), Object(P) !== P)) return;
            } finally {
              if (W) throw S;
            }
          }
          return k;
        }
      }
      function E(f) {
        if (Array.isArray(f)) return f;
      }
      function N(f) {
        var M = f.pathname,
          i = f.current,
          p = f.target,
          S = 'base' in i ? M.replace(i.base.replace(/\/$/, ''), '') || '/' : M.replace(new RegExp(''.concat(i.suffix, '$')), '');
        return 'base' in p
          ? ''
              .concat(p.base.replace(/\/$/, ''))
              .concat(S)
              .replace(/([^/])\/$/, '$1')
          : ''.concat(S).concat(p.suffix);
      }
      var y = function (M) {
          var i = M.locale,
            p = M.current,
            S = (0, I.TH)(),
            b = S.pathname,
            P = (0, v.useState)(function () {
              return N({ pathname: b, current: p, target: i });
            }),
            k = O(P, 2),
            U = k[0],
            W = k[1];
          return (
            (0, v.useEffect)(
              function () {
                W(N({ pathname: b, current: p, target: i }));
              },
              [b, p.id, i.id],
            ),
            v.createElement(I.rU, { className: 'dumi-default-lang-switch', to: U }, i.name)
          );
        },
        D = function () {
          var M = (0, I.WF)(),
            i = M.locales,
            p = (0, I.YB)(),
            S = p.locale,
            b = (0, I.bU)();
          return i.length <= 1
            ? null
            : i.length > 2
            ? v.createElement(
                'div',
                { className: 'dumi-default-lang-select' },
                v.createElement(
                  'select',
                  {
                    defaultValue: S,
                    onChange: function (k) {
                      I.m8.push(
                        N({
                          pathname: I.m8.location.pathname,
                          current: b,
                          target: i.find(function (U) {
                            var W = U.id;
                            return W === k.target.value;
                          }),
                        }),
                      );
                    },
                  },
                  i.map(function (P) {
                    return v.createElement('option', { key: P.id, value: P.id }, P.name);
                  }),
                ),
                v.createElement(e.r, null),
              )
            : v.createElement(y, {
                locale: i.find(function (P) {
                  var k = P.id;
                  return k !== S;
                }),
                current: b,
              });
        },
        x = D;
    },
    66265: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return O;
        },
      });
      var e = r(80904),
        I = r(70079),
        v = function () {
          var z = (0, e.WF)(),
            Q = z.themeConfig,
            T = (0, e.bU)();
          return I.createElement(
            e.rU,
            { className: 'dumi-default-logo', to: 'base' in T ? T.base : '/' },
            Q.logo !== !1
              && I.createElement('img', {
                src:
                  Q.logo
                  || 'data:image/png;base64,',
                alt: Q.name,
              }),
            Q.name,
          );
        },
        O = v;
    },
    51669: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return x;
        },
      });
      var e = r(80904),
        I = r(70079);
      function v(f, M) {
        return T(f) || Q(f, M) || w(f, M) || O();
      }
      function O() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function w(f, M) {
        if (f) {
          if (typeof f == 'string') return z(f, M);
          var i = Object.prototype.toString.call(f).slice(8, -1);
          if ((i === 'Object' && f.constructor && (i = f.constructor.name), i === 'Map' || i === 'Set')) return Array.from(f);
          if (i === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(i)) return z(f, M);
        }
      }
      function z(f, M) {
        (M == null || M > f.length) && (M = f.length);
        for (var i = 0, p = new Array(M); i < M; i++) p[i] = f[i];
        return p;
      }
      function Q(f, M) {
        var i = f == null ? null : (typeof Symbol != 'undefined' && f[Symbol.iterator]) || f['@@iterator'];
        if (i != null) {
          var p,
            S,
            b,
            P,
            k = [],
            U = !0,
            W = !1;
          try {
            if (((b = (i = i.call(f)).next), M === 0)) {
              if (Object(i) !== i) return;
              U = !1;
            } else for (; !(U = (p = b.call(i)).done) && (k.push(p.value), k.length !== M); U = !0);
          } catch (B) {
            (W = !0), (S = B);
          } finally {
            try {
              if (!U && i.return != null && ((P = i.return()), Object(P) !== P)) return;
            } finally {
              if (W) throw S;
            }
          }
          return k;
        }
      }
      function T(f) {
        if (Array.isArray(f)) return f;
      }
      var E = function () {
          return I.createElement(
            'svg',
            { viewBox: '0 0 14 16' },
            I.createElement('path', {
              d: 'M5.003 6.39v3.594c0 .4.275.674.674.674.4 0 .674-.274.674-.674V1.323h1.997v8.661c0 .4.274.674.674.674s.674-.274.674-.674V1.323h3.295c.399 0 .674-.275.674-.674 0-.4-.275-.649-.674-.649H4.928C3.131 0 1.733 1.398 1.733 3.195S3.206 6.39 5.003 6.39Zm0-5.067v3.72c-1.073 0-1.922-.8-1.922-1.873s.799-1.847 1.922-1.847Zm7.988 11.332H2.73l.8-.674c.274-.2.324-.674.124-.923-.2-.275-.674-.325-.923-.125L.735 12.53c-.275.275-.4.525-.4.874 0 .325.125.674.4.874l1.997 1.597a.829.829 0 0 0 .4.125c.199 0 .398-.075.523-.275.2-.274.2-.723-.125-.923l-.998-.799h10.459c.399 0 .674-.274.674-.674 0-.424-.275-.674-.674-.674Z',
            }),
          );
        },
        N = function () {
          return I.createElement(
            'svg',
            { viewBox: '0 0 14 16' },
            I.createElement('path', {
              d: 'M5.003 6.39v3.594c0 .4.275.674.674.674.4 0 .674-.274.674-.674V1.323h1.997v8.661c0 .4.274.674.674.674s.674-.274.674-.674V1.323h3.295c.399 0 .674-.275.674-.674 0-.4-.275-.649-.674-.649H4.928C3.131 0 1.733 1.398 1.733 3.195S3.206 6.39 5.003 6.39Zm0-5.067v3.72c-1.073 0-1.922-.8-1.922-1.873s.799-1.847 1.922-1.847ZM1.01 12.655h10.26l-.8-.674c-.274-.2-.324-.674-.124-.923.2-.275.674-.325.923-.125l1.997 1.597c.275.275.4.525.4.874 0 .325-.125.674-.4.874l-1.997 1.597a.829.829 0 0 1-.399.125.59.59 0 0 1-.524-.275c-.2-.274-.2-.723.125-.923l.998-.799H1.009c-.399 0-.674-.274-.674-.674 0-.424.275-.674.674-.674Z',
            }),
          );
        },
        y = 'dumi:rtl',
        D = function () {
          var M = (0, I.useState)(!1),
            i = v(M, 2),
            p = i[0],
            S = i[1],
            b = (0, e.WF)(),
            P = b.themeConfig;
          return (
            (0, I.useEffect)(function () {
              localStorage.getItem(y) && (S(!0), document.documentElement.setAttribute('data-direction', 'rtl'));
            }, []),
            P.rtl
              ? I.createElement(
                  'button',
                  {
                    type: 'button',
                    className: 'dumi-default-rtl-switch',
                    onClick: function () {
                      p
                        ? (document.documentElement.removeAttribute('data-direction'), localStorage.removeItem(y))
                        : (document.documentElement.setAttribute('data-direction', 'rtl'), localStorage.setItem(y, '1')),
                        S(!p);
                    },
                    'data-dumi-tooltip': p ? 'RTL' : 'LTR',
                    'data-dumi-tooltip-bottom': !0,
                  },
                  p ? I.createElement(E, null) : I.createElement(N, null),
                )
              : null
          );
        },
        x = D;
    },
    25995: function (j, h, r) {
      'use strict';
      r.d(h, {
        ZP: function () {
          return Y;
        },
      });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (u, c, l) => (c in u ? I(u, c, { enumerable: !0, configurable: !0, writable: !0, value: l }) : (u[c] = l)),
        Q = (u, c) => {
          for (var l in c || (c = {})) O.call(c, l) && z(u, l, c[l]);
          if (v) for (var l of v(c)) w.call(c, l) && z(u, l, c[l]);
          return u;
        };
      const T = (u) =>
        e.createElement(
          'svg',
          Q({ viewBox: '64 64 896 896' }, u),
          e.createElement('path', {
            d: 'M862 465.3h-81c-4.6 0-9 2-12.1 5.5L550 723.1V160c0-4.4-3.6-8-8-8h-60c-4.4 0-8 3.6-8 8v563.1L255.1 470.8c-3-3.5-7.4-5.5-12.1-5.5h-81c-6.8 0-10.5 8.1-6 13.2L487.9 861a31.96 31.96 0 0 0 48.3 0L868 478.5c4.5-5.2.8-13.2-6-13.2z',
          }),
        );
      var E =
          'data:image/svg+xml;base64,',
        N = Object.defineProperty,
        y = Object.getOwnPropertySymbols,
        D = Object.prototype.hasOwnProperty,
        x = Object.prototype.propertyIsEnumerable,
        f = (u, c, l) => (c in u ? N(u, c, { enumerable: !0, configurable: !0, writable: !0, value: l }) : (u[c] = l)),
        M = (u, c) => {
          for (var l in c || (c = {})) D.call(c, l) && f(u, l, c[l]);
          if (y) for (var l of y(c)) x.call(c, l) && f(u, l, c[l]);
          return u;
        };
      const i = (u) =>
        e.createElement(
          'svg',
          M({ viewBox: '64 64 896 896' }, u),
          e.createElement('path', {
            d: 'M868 545.5 536.1 163a31.96 31.96 0 0 0-48.3 0L156 545.5a7.97 7.97 0 0 0 6 13.2h81c4.6 0 9-2 12.1-5.5L474 300.9V864c0 4.4 3.6 8 8 8h60c4.4 0 8-3.6 8-8V300.9l218.9 252.3c3 3.5 7.4 5.5 12.1 5.5h81c6.8 0 10.5-8 6-13.2z',
          }),
        );
      var p =
          'data:image/svg+xml;base64,',
        S = Object.defineProperty,
        b = Object.getOwnPropertySymbols,
        P = Object.prototype.hasOwnProperty,
        k = Object.prototype.propertyIsEnumerable,
        U = (u, c, l) => (c in u ? S(u, c, { enumerable: !0, configurable: !0, writable: !0, value: l }) : (u[c] = l)),
        W = (u, c) => {
          for (var l in c || (c = {})) P.call(c, l) && U(u, l, c[l]);
          if (b) for (var l of b(c)) k.call(c, l) && U(u, l, c[l]);
          return u;
        };
      const B = (u) =>
        e.createElement(
          'svg',
          W({ viewBox: '64 64 896 896' }, u),
          e.createElement('path', {
            d: 'M909.6 854.5 649.9 594.8C690.2 542.7 712 479 712 412c0-80.2-31.3-155.4-87.9-212.1-56.6-56.7-132-87.9-212.1-87.9s-155.5 31.3-212.1 87.9C143.2 256.5 112 331.8 112 412c0 80.1 31.3 155.5 87.9 212.1C256.5 680.8 331.8 712 412 712c67 0 130.6-21.8 182.7-62l259.7 259.6a8.2 8.2 0 0 0 11.6 0l43.6-43.5a8.2 8.2 0 0 0 0-11.6zM570.4 570.4C528 612.7 471.8 636 412 636s-116-23.3-158.4-65.6C211.3 528 188 471.8 188 412s23.3-116.1 65.6-158.4C296 211.3 352.2 188 412 188s116.1 23.2 158.4 65.6S636 352.2 636 412s-23.3 116.1-65.6 158.4z',
          }),
        );
      var R =
          'data:image/svg+xml;base64,',
        C = r(80904),
        A = r(89471),
        o = (0, e.forwardRef)(function (u, c) {
          var l = (0, C.YB)(),
            F = (0, e.useRef)(!1),
            Z = (0, e.useRef)(null);
          return (
            (0, e.useImperativeHandle)(c, function () {
              return Z.current;
            }),
            e.createElement('input', {
              className: 'dumi-default-search-bar-input',
              onCompositionStart: function () {
                return (F.current = !0);
              },
              onCompositionEnd: function ($) {
                (F.current = !1), u.onChange($.currentTarget.value);
              },
              onFocus: u.onFocus,
              onBlur: u.onBlur,
              onKeyDown: function ($) {
                ['ArrowDown', 'ArrowUp'].includes($.key) && $.preventDefault(), $.key === 'Escape' && !F.current && $.currentTarget.blur();
              },
              onChange: function ($) {
                var te = $.target.value;
                setTimeout(function () {
                  F.current || u.onChange(te);
                }, 1);
              },
              placeholder: l.formatMessage({ id: 'header.search.placeholder' }),
              ref: Z,
            })
          );
        }),
        g = function (c) {
          return (
            (0, e.useEffect)(
              function () {
                if (c.visible) document.body.style.overflow = 'hidden';
                else {
                  var l;
                  (document.body.style.overflow = ''), (l = c.onClose) === null || l === void 0 || l.call(c);
                }
              },
              [c.visible],
            ),
            c.visible
              ? e.createElement(
                  'div',
                  { className: 'dumi-default-search-modal' },
                  e.createElement('div', { className: 'dumi-default-search-modal-mask', onClick: c.onMaskClick }),
                  e.createElement('div', { className: 'dumi-default-search-modal-content' }, c.children),
                )
              : null
          );
        },
        d;
      function V(u, c) {
        return n(u) || t(u, c) || X(u, c) || J();
      }
      function J() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function X(u, c) {
        if (u) {
          if (typeof u == 'string') return K(u, c);
          var l = Object.prototype.toString.call(u).slice(8, -1);
          if ((l === 'Object' && u.constructor && (l = u.constructor.name), l === 'Map' || l === 'Set')) return Array.from(u);
          if (l === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(l)) return K(u, c);
        }
      }
      function K(u, c) {
        (c == null || c > u.length) && (c = u.length);
        for (var l = 0, F = new Array(c); l < c; l++) F[l] = u[l];
        return F;
      }
      function t(u, c) {
        var l = u == null ? null : (typeof Symbol != 'undefined' && u[Symbol.iterator]) || u['@@iterator'];
        if (l != null) {
          var F,
            Z,
            _,
            $,
            te = [],
            ee = !0,
            ne = !1;
          try {
            if (((_ = (l = l.call(u)).next), c === 0)) {
              if (Object(l) !== l) return;
              ee = !1;
            } else for (; !(ee = (F = _.call(l)).done) && (te.push(F.value), te.length !== c); ee = !0);
          } catch (ce) {
            (ne = !0), (Z = ce);
          } finally {
            try {
              if (!ee && l.return != null && (($ = l.return()), Object($) !== $)) return;
            } finally {
              if (ne) throw Z;
            }
          }
          return te;
        }
      }
      function n(u) {
        if (Array.isArray(u)) return u;
      }
      var a = /(mac|iphone|ipod|ipad)/i.test(
          typeof navigator != 'undefined' ? ((d = navigator) === null || d === void 0 ? void 0 : d.platform) : '',
        ),
        G = function (c) {
          return ['TEXTAREA', 'INPUT'].includes(c.tagName) || c.contentEditable === 'true';
        },
        H = function () {
          var c = (0, e.useState)(!1),
            l = V(c, 2),
            F = l[0],
            Z = l[1],
            _ = (0, e.useRef)(null),
            $ = (0, e.useRef)(null),
            te = (0, e.useState)('\u2318'),
            ee = V(te, 2),
            ne = ee[0],
            ce = ee[1],
            ae = (0, C.OO)(),
            se = ae.keywords,
            oe = ae.setKeywords,
            le = ae.result,
            ie = ae.loading,
            Me = (0, e.useState)(!1),
            fe = V(Me, 2),
            ge = fe[0],
            ue = fe[1];
          return (
            (0, e.useEffect)(function () {
              a || ce('Ctrl');
              var q = function (L) {
                if (
                  (((a ? L.metaKey : L.ctrlKey) && L.key === 'k') || (L.key === '/' && !G(L.target)))
                  && (L.preventDefault(), _.current)
                ) {
                  var s = _.current.getBoundingClientRect(),
                    m = s.top,
                    ye = s.bottom,
                    me = s.left,
                    Ae = s.right,
                    Ne = m >= 0 && me >= 0 && ye <= window.innerHeight && Ae <= window.innerWidth;
                  Ne
                    ? _.current.focus()
                    : (oe(''),
                      ue(!0),
                      setTimeout(function () {
                        var Le;
                        (Le = $.current) === null || Le === void 0 || Le.focus();
                      }));
                }
                L.key === 'Escape' && (L.preventDefault(), ue(!1));
              };
              return (
                document.addEventListener('keydown', q),
                function () {
                  return document.removeEventListener('keydown', q);
                }
              );
            }, []),
            e.createElement(
              'div',
              { className: 'dumi-default-search-bar' },
              e.createElement(B, { className: 'dumi-default-search-bar-svg' }),
              e.createElement(o, {
                onFocus: function () {
                  return Z(!0);
                },
                onBlur: function () {
                  setTimeout(function () {
                    Z(!1);
                  }, 1);
                },
                onChange: function (re) {
                  return oe(re);
                },
                ref: _,
              }),
              e.createElement('span', { className: 'dumi-default-search-shortcut' }, ne, ' K'),
              se.trim()
                && F
                && (le.length || !ie)
                && !ge
                && e.createElement(
                  'div',
                  { className: 'dumi-default-search-popover' },
                  e.createElement('section', null, e.createElement(A.Z, { data: le, loading: ie })),
                ),
              e.createElement(
                g,
                {
                  visible: ge,
                  onMaskClick: function () {
                    ue(!1);
                  },
                  onClose: function () {
                    return oe('');
                  },
                },
                e.createElement(
                  'div',
                  { style: { position: 'relative' } },
                  e.createElement(B, { className: 'dumi-default-search-bar-svg' }),
                  e.createElement(o, {
                    onFocus: function () {
                      return Z(!0);
                    },
                    onBlur: function () {
                      setTimeout(function () {
                        Z(!1);
                      }, 1);
                    },
                    onChange: function (re) {
                      return oe(re);
                    },
                    ref: $,
                  }),
                ),
                e.createElement(A.Z, {
                  data: le,
                  loading: ie,
                  onItemSelect: function () {
                    ue(!1);
                  },
                }),
                e.createElement(
                  'footer',
                  null,
                  e.createElement(
                    'ul',
                    { className: 'dumi-default-search-modal-commands' },
                    e.createElement(
                      'li',
                      { className: 'dumi-default-search-modal-commands-arrow' },
                      e.createElement(
                        'span',
                        { className: 'dumi-default-search-modal-shortcut' },
                        e.createElement(i, { width: '10px', height: '10px', fill: 'rgba(0, 0, 0, 0.45)' }),
                      ),
                      e.createElement(
                        'span',
                        { className: 'dumi-default-search-modal-shortcut' },
                        e.createElement(T, { width: '10px', height: '10px', fill: 'rgba(0, 0, 0, 0.45)' }),
                      ),
                      e.createElement('span', { className: 'dumi-default-search-modal-commands-text' }, 'to navigate'),
                    ),
                    e.createElement(
                      'li',
                      null,
                      e.createElement('span', { className: 'dumi-default-search-modal-shortcut' }, 'esc'),
                      e.createElement('span', { className: 'dumi-default-search-modal-commands-text' }, 'to close'),
                    ),
                  ),
                ),
              ),
            )
          );
        },
        Y = H;
    },
    20406: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return C;
        },
      });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (A, o, g) => (o in A ? I(A, o, { enumerable: !0, configurable: !0, writable: !0, value: g }) : (A[o] = g)),
        Q = (A, o) => {
          for (var g in o || (o = {})) O.call(o, g) && z(A, g, o[g]);
          if (v) for (var g of v(o)) w.call(o, g) && z(A, g, o[g]);
          return A;
        };
      const T = (A) =>
        e.createElement(
          'svg',
          Q({ viewBox: '0 0 1024 1024' }, A),
          e.createElement('path', {
            d: 'm885.2 446.3-.2-.8-112.2-285.1c-5-16.1-19.9-27.2-36.8-27.2H281.2c-17 0-32.1 11.3-36.9 27.6L139.4 443l-.3.7-.2.8c-1.3 4.9-1.7 9.9-1 14.8-.1 1.6-.2 3.2-.2 4.8V830a60.9 60.9 0 0 0 60.8 60.8h627.2c33.5 0 60.8-27.3 60.9-60.8V464.1c0-1.3 0-2.6-.1-3.7.4-4.9 0-9.6-1.3-14.1zm-295.8-43-.3 15.7c-.8 44.9-31.8 75.1-77.1 75.1-22.1 0-41.1-7.1-54.8-20.6S436 441.2 435.6 419l-.3-15.7H229.5L309 210h399.2l81.7 193.3H589.4zm-375 76.8h157.3c24.3 57.1 76 90.8 140.4 90.8 33.7 0 65-9.4 90.3-27.2 22.2-15.6 39.5-37.4 50.7-63.6h156.5V814H214.4V480.1z',
          }),
        );
      var E =
          'data:image/svg+xml;base64,',
        N = r(22484),
        y = r(80904);
      function D(A, o) {
        return p(A) || i(A, o) || f(A, o) || x();
      }
      function x() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function f(A, o) {
        if (A) {
          if (typeof A == 'string') return M(A, o);
          var g = Object.prototype.toString.call(A).slice(8, -1);
          if ((g === 'Object' && A.constructor && (g = A.constructor.name), g === 'Map' || g === 'Set')) return Array.from(A);
          if (g === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(g)) return M(A, o);
        }
      }
      function M(A, o) {
        (o == null || o > A.length) && (o = A.length);
        for (var g = 0, d = new Array(o); g < o; g++) d[g] = A[g];
        return d;
      }
      function i(A, o) {
        var g = A == null ? null : (typeof Symbol != 'undefined' && A[Symbol.iterator]) || A['@@iterator'];
        if (g != null) {
          var d,
            V,
            J,
            X,
            K = [],
            t = !0,
            n = !1;
          try {
            if (((J = (g = g.call(A)).next), o === 0)) {
              if (Object(g) !== g) return;
              t = !1;
            } else for (; !(t = (d = J.call(g)).done) && (K.push(d.value), K.length !== o); t = !0);
          } catch (a) {
            (n = !0), (V = a);
          } finally {
            try {
              if (!t && g.return != null && ((X = g.return()), Object(X) !== X)) return;
            } finally {
              if (n) throw V;
            }
          }
          return K;
        }
      }
      function p(A) {
        if (Array.isArray(A)) return A;
      }
      var S = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M5.333 10.667h21.334c.889 0 1.333.444 1.333 1.333s-.444 1.333-1.333 1.333H5.333C4.444 13.333 4 12.89 4 12s.444-1.333 1.333-1.333Z',
            }),
            e.createElement('path', {
              d: 'M13.207 2.667h.126a1.206 1.206 0 0 1 1.2 1.326l-2.413 24.14a1.333 1.333 0 0 1-1.327 1.2h-.126a1.206 1.206 0 0 1-1.2-1.326l2.413-24.14c.068-.682.642-1.2 1.327-1.2Zm8 0h.126a1.206 1.206 0 0 1 1.2 1.326l-2.413 24.14a1.333 1.333 0 0 1-1.327 1.2h-.126a1.206 1.206 0 0 1-1.2-1.326l2.413-24.14c.068-.682.642-1.2 1.327-1.2Z',
            }),
            e.createElement('path', {
              d: 'M5.333 18.667h21.334c.889 0 1.333.444 1.333 1.333s-.444 1.333-1.333 1.333H5.333C4.444 21.333 4 20.89 4 20s.444-1.333 1.333-1.333Z',
            }),
          );
        },
        b = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M9.402 0h14.78L30 6.16V24.5c0 1.933-1.71 3.5-3.589 3.5H9.401C7.524 28 6 26.433 6 24.5v-21C6 1.567 7.523 0 9.402 0ZM23 2v4.183c0 .451.366.817.817.817H28l-5-5Zm3.333 24c.92 0 1.667-.768 1.667-1.714V8.857h-5c-.92 0-1.667-.767-1.667-1.714V2H9.667C8.747 2 8 2.768 8 3.714v20.572C8 25.232 8.746 26 9.667 26h16.666Z',
            }),
          );
        },
        P = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M6.12 14.589h6.628l1.52 4.004h2.485l-5.938-15.19H8.053L2.115 18.732H4.6l1.52-4.143ZM8.88 6.855c.139-.414.277-.828.415-1.38h.138c0 .138.138.414.414 1.104 0 .138.138.276.138.276 0 .138.829 2.072 2.21 5.938H6.672c1.519-3.866 2.21-5.8 2.21-5.938Zm8.148 2.348h12.705v1.933H17.029V9.203ZM2.115 20.665h27.619v1.933H2.114v-1.933Zm14.914-5.662h12.705v1.933H17.029v-1.933ZM2.115 26.327h27.619v1.933H2.114v-1.933ZM17.029 3.54h12.705v1.934H17.029V3.54Z',
            }),
          );
        },
        k = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M28 6h-5a5 5 0 0 0-10 0H8a2 2 0 0 0-2 2v5a5 5 0 0 0 0 10v5a2 2 0 0 0 2 2h7v-2a3 3 0 0 1 6 0v2h7a2 2 0 0 0 2-2v-7h-2a3 3 0 0 1 0-6h2V8a2 2 0 0 0-2-2Zm-5 12a5 5 0 0 0 5 5v5h-5a5 5 0 0 0-10 0H8v-7H6a3 3 0 0 1 0-6h2V8h7V6a3 3 0 0 1 6 0v2h7v5a5 5 0 0 0-5 5Z',
            }),
          );
        },
        U = { title: S, page: b, content: P, demo: k },
        W = function (o) {
          return e.createElement(
            e.Fragment,
            null,
            o.texts.map(function (g, d) {
              return e.createElement(e.Fragment, { key: d }, g.highlighted ? e.createElement('mark', null, g.text) : g.text);
            }),
          );
        },
        B = function (o) {
          var g = (0, e.useCallback)(
              function () {
                var K = 0,
                  t = [];
                return (
                  o.forEach(function (n) {
                    n.title && t.push({ type: 'title', value: { title: n.title } }),
                      n.hints.forEach(function (a) {
                        t.push({ type: 'hint', activeIndex: K++, value: a });
                      });
                  }),
                  [t, K]
                );
              },
              [o],
            ),
            d = (0, e.useState)(g),
            V = D(d, 2),
            J = V[0],
            X = V[1];
          return (
            (0, e.useEffect)(
              function () {
                X(g);
              },
              [o],
            ),
            J
          );
        },
        R = function (o) {
          var g = B(o.data),
            d = D(g, 2),
            V = d[0],
            J = d[1],
            X = (0, e.useState)(-1),
            K = D(X, 2),
            t = K[0],
            n = K[1],
            a = (0, y.TH)(),
            G = a.pathname,
            H = function (u) {
              var c;
              (c = o.onItemSelect) === null || c === void 0 || c.call(o, u);
              var l = new URL(u == null ? void 0 : u.link, location.origin);
              (l == null ? void 0 : l.pathname) === G
                && !l.hash
                && setTimeout(function () {
                  (0, N.Z)(0, { maxDuration: 300 });
                }, 1);
            };
          return (
            (0, e.useEffect)(function () {
              var Y = function (c) {
                if (c.key === 'ArrowDown') n((t + 1) % J);
                else if (c.key === 'ArrowUp') n((t + J - 1) % J);
                else if (c.key === 'Enter' && t >= 0) {
                  var l = V.find(function (F) {
                    return F.type === 'hint' && F.activeIndex === t;
                  }).value;
                  y.m8.push(l.link), H == null || H(l), document.activeElement.blur();
                }
                ['Escape', 'Enter'].includes(c.key) && n(-1);
              };
              return (
                document.addEventListener('keydown', Y),
                function () {
                  return document.removeEventListener('keydown', Y);
                }
              );
            }),
            e.createElement(
              'div',
              {
                className: 'dumi-default-search-result',
                onMouseEnter: function () {
                  return n(-1);
                },
                onMouseDownCapture: function (u) {
                  return u.preventDefault();
                },
                onMouseUpCapture: function () {
                  document.activeElement.blur();
                },
              },
              o.data.length || o.loading
                ? e.createElement(
                    'dl',
                    null,
                    V.map(function (Y, u) {
                      return Y.type === 'title'
                        ? e.createElement('dt', { key: String(u) }, Y.value.title)
                        : e.createElement(
                            'dd',
                            { key: String(u) },
                            e.createElement(
                              y.rU,
                              {
                                to: Y.value.link,
                                'data-active': t === Y.activeIndex || void 0,
                                onClick: function () {
                                  return H == null ? void 0 : H(Y.value);
                                },
                              },
                              e.createElement(U[Y.value.type]),
                              e.createElement('h4', null, e.createElement(W, { texts: Y.value.highlightTitleTexts })),
                              e.createElement('p', null, e.createElement(W, { texts: Y.value.highlightTexts })),
                            ),
                          );
                    }),
                  )
                : e.createElement(
                    'div',
                    { className: 'dumi-default-search-empty' },
                    e.createElement(T, null),
                    e.createElement(y._H, { id: 'search.not.found' }),
                  ),
            )
          );
        },
        C = R;
    },
    99419: function (j, h, r) {
      'use strict';
      r.d(h, {
        Z: function () {
          return re;
        },
      });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (L, s, m) => (s in L ? I(L, s, { enumerable: !0, configurable: !0, writable: !0, value: m }) : (L[s] = m)),
        Q = (L, s) => {
          for (var m in s || (s = {})) O.call(s, m) && z(L, m, s[m]);
          if (v) for (var m of v(s)) w.call(s, m) && z(L, m, s[m]);
          return L;
        };
      const T = (L) =>
        e.createElement(
          'svg',
          Q({ viewBox: '64 64 896 896' }, L),
          e.createElement('path', {
            d: 'M880 112H144c-17.7 0-32 14.3-32 32v736c0 17.7 14.3 32 32 32h736c17.7 0 32-14.3 32-32V144c0-17.7-14.3-32-32-32zm-32 736H663.9V602.2h104l15.6-120.7H663.9v-77.1c0-35 9.7-58.8 59.8-58.8h63.9v-108c-11.1-1.5-49-4.8-93.2-4.8-92.2 0-155.3 56.3-155.3 159.6v89H434.9v120.7h104.3V848H176V176h672v672z',
          }),
        );
      var E =
          'data:image/svg+xml;base64,',
        N = r(19444),
        y = Object.defineProperty,
        D = Object.getOwnPropertySymbols,
        x = Object.prototype.hasOwnProperty,
        f = Object.prototype.propertyIsEnumerable,
        M = (L, s, m) => (s in L ? y(L, s, { enumerable: !0, configurable: !0, writable: !0, value: m }) : (L[s] = m)),
        i = (L, s) => {
          for (var m in s || (s = {})) x.call(s, m) && M(L, m, s[m]);
          if (D) for (var m of D(s)) f.call(s, m) && M(L, m, s[m]);
          return L;
        };
      const p = (L) =>
        e.createElement(
          'svg',
          i({ viewBox: '64 64 896 896' }, L),
          e.createElement('path', {
            d: 'M913.9 552.2 805 181.4v-.1c-7.6-22.9-25.7-36.5-48.3-36.5-23.4 0-42.5 13.5-49.7 35.2l-71.4 213H388.8l-71.4-213c-7.2-21.7-26.3-35.2-49.7-35.2-23.1 0-42.5 14.8-48.4 36.6L110.5 552.2c-4.4 14.7 1.2 31.4 13.5 40.7l368.5 276.4c2.6 3.6 6.2 6.3 10.4 7.8l8.6 6.4 8.5-6.4c4.9-1.7 9-4.7 11.9-8.9l368.4-275.4c12.4-9.2 18-25.9 13.6-40.6zM751.7 193.4c1-1.8 2.9-1.9 3.5-1.9 1.1 0 2.5.3 3.4 3L818 394.3H684.5l67.2-200.9zm-487.4 1c.9-2.6 2.3-2.9 3.4-2.9 2.7 0 2.9.1 3.4 1.7l67.3 201.2H206.5l57.8-200zM158.8 558.7l28.2-97.3 202.4 270.2-230.6-172.9zm73.9-116.4h122.1l90.8 284.3-212.9-284.3zM512.9 776 405.7 442.3H620L512.9 776zm157.9-333.7h119.5L580 723.1l90.8-280.8zm-40.7 293.9 207.3-276.7 29.5 99.2-236.8 177.5z',
          }),
        );
      var S =
          'data:image/svg+xml;base64,',
        b = Object.defineProperty,
        P = Object.getOwnPropertySymbols,
        k = Object.prototype.hasOwnProperty,
        U = Object.prototype.propertyIsEnumerable,
        W = (L, s, m) => (s in L ? b(L, s, { enumerable: !0, configurable: !0, writable: !0, value: m }) : (L[s] = m)),
        B = (L, s) => {
          for (var m in s || (s = {})) k.call(s, m) && W(L, m, s[m]);
          if (P) for (var m of P(s)) U.call(s, m) && W(L, m, s[m]);
          return L;
        };
      const R = (L) =>
        e.createElement(
          'svg',
          B({ viewBox: '64 64 896 896' }, L),
          e.createElement('path', {
            d: 'M847.7 112H176.3c-35.5 0-64.3 28.8-64.3 64.3v671.4c0 35.5 28.8 64.3 64.3 64.3h671.4c35.5 0 64.3-28.8 64.3-64.3V176.3c0-35.5-28.8-64.3-64.3-64.3zm0 736c-447.8-.1-671.7-.2-671.7-.3.1-447.8.2-671.7.3-671.7 447.8.1 671.7.2 671.7.3-.1 447.8-.2 671.7-.3 671.7zM230.6 411.9h118.7v381.8H230.6zm59.4-52.2c37.9 0 68.8-30.8 68.8-68.8a68.8 68.8 0 1 0-137.6 0c-.1 38 30.7 68.8 68.8 68.8zm252.3 245.1c0-49.8 9.5-98 71.2-98 60.8 0 61.7 56.9 61.7 101.2v185.7h118.6V584.3c0-102.8-22.2-181.9-142.3-181.9-57.7 0-96.4 31.7-112.3 61.7h-1.6v-52.2H423.7v381.8h118.6V604.8z',
          }),
        );
      var C =
          'data:image/svg+xml;base64,',
        A = Object.defineProperty,
        o = Object.getOwnPropertySymbols,
        g = Object.prototype.hasOwnProperty,
        d = Object.prototype.propertyIsEnumerable,
        V = (L, s, m) => (s in L ? A(L, s, { enumerable: !0, configurable: !0, writable: !0, value: m }) : (L[s] = m)),
        J = (L, s) => {
          for (var m in s || (s = {})) g.call(s, m) && V(L, m, s[m]);
          if (o) for (var m of o(s)) d.call(s, m) && V(L, m, s[m]);
          return L;
        };
      const X = (L) =>
        e.createElement(
          'svg',
          J({ viewBox: '64 64 896 896' }, L),
          e.createElement('path', {
            d: 'M928 254.3c-30.6 13.2-63.9 22.7-98.2 26.4a170.1 170.1 0 0 0 75-94 336.64 336.64 0 0 1-108.2 41.2A170.1 170.1 0 0 0 672 174c-94.5 0-170.5 76.6-170.5 170.6 0 13.2 1.6 26.4 4.2 39.1-141.5-7.4-267.7-75-351.6-178.5a169.32 169.32 0 0 0-23.2 86.1c0 59.2 30.1 111.4 76 142.1a172 172 0 0 1-77.1-21.7v2.1c0 82.9 58.6 151.6 136.7 167.4a180.6 180.6 0 0 1-44.9 5.8c-11.1 0-21.6-1.1-32.2-2.6C211 652 273.9 701.1 348.8 702.7c-58.6 45.9-132 72.9-211.7 72.9-14.3 0-27.5-.5-41.2-2.1C171.5 822 261.2 850 357.8 850 671.4 850 843 590.2 843 364.7c0-7.4 0-14.8-.5-22.2 33.2-24.3 62.3-54.4 85.5-88.2z',
          }),
        );
      var K =
          'data:image/svg+xml;base64,',
        t = Object.defineProperty,
        n = Object.getOwnPropertySymbols,
        a = Object.prototype.hasOwnProperty,
        G = Object.prototype.propertyIsEnumerable,
        H = (L, s, m) => (s in L ? t(L, s, { enumerable: !0, configurable: !0, writable: !0, value: m }) : (L[s] = m)),
        Y = (L, s) => {
          for (var m in s || (s = {})) a.call(s, m) && H(L, m, s[m]);
          if (n) for (var m of n(s)) G.call(s, m) && H(L, m, s[m]);
          return L;
        };
      const u = (L) =>
        e.createElement(
          'svg',
          Y({ viewBox: '64 64 896 896' }, L),
          e.createElement('path', {
            d: 'M457.3 543c-68.1-17.7-145 16.2-174.6 76.2-30.1 61.2-1 129.1 67.8 151.3 71.2 23 155.2-12.2 184.4-78.3 28.7-64.6-7.2-131-77.6-149.2zm-52 156.2c-13.8 22.1-43.5 31.7-65.8 21.6-22-10-28.5-35.7-14.6-57.2 13.7-21.4 42.3-31 64.4-21.7 22.4 9.5 29.6 35 16 57.3zm45.5-58.5c-5 8.6-16.1 12.7-24.7 9.1-8.5-3.5-11.2-13.1-6.4-21.5 5-8.4 15.6-12.4 24.1-9.1 8.7 3.2 11.8 12.9 7 21.5zm334.5-197.2c15 4.8 31-3.4 35.9-18.3 11.8-36.6 4.4-78.4-23.2-109a111.39 111.39 0 0 0-106-34.3 28.45 28.45 0 0 0-21.9 33.8 28.39 28.39 0 0 0 33.8 21.8c18.4-3.9 38.3 1.8 51.9 16.7a54.2 54.2 0 0 1 11.3 53.3 28.45 28.45 0 0 0 18.2 36zm99.8-206c-56.7-62.9-140.4-86.9-217.7-70.5a32.98 32.98 0 0 0-25.4 39.3 33.12 33.12 0 0 0 39.3 25.5c55-11.7 114.4 5.4 154.8 50.1 40.3 44.7 51.2 105.7 34 159.1-5.6 17.4 3.9 36 21.3 41.7 17.4 5.6 36-3.9 41.6-21.2v-.1c24.1-75.4 8.9-161.1-47.9-223.9zM729 499c-12.2-3.6-20.5-6.1-14.1-22.1 13.8-34.7 15.2-64.7.3-86-28-40.1-104.8-37.9-192.8-1.1 0 0-27.6 12.1-20.6-9.8 13.5-43.5 11.5-79.9-9.6-101-47.7-47.8-174.6 1.8-283.5 110.6C127.3 471.1 80 557.5 80 632.2 80 775.1 263.2 862 442.5 862c235 0 391.3-136.5 391.3-245 0-65.5-55.2-102.6-104.8-118zM443 810.8c-143 14.1-266.5-50.5-275.8-144.5-9.3-93.9 99.2-181.5 242.2-195.6 143-14.2 266.5 50.5 275.8 144.4C694.4 709 586 796.6 443 810.8z',
          }),
        );
      var c =
          'data:image/svg+xml;base64,',
        l = Object.defineProperty,
        F = Object.getOwnPropertySymbols,
        Z = Object.prototype.hasOwnProperty,
        _ = Object.prototype.propertyIsEnumerable,
        $ = (L, s, m) => (s in L ? l(L, s, { enumerable: !0, configurable: !0, writable: !0, value: m }) : (L[s] = m)),
        te = (L, s) => {
          for (var m in s || (s = {})) Z.call(s, m) && $(L, m, s[m]);
          if (F) for (var m of F(s)) _.call(s, m) && $(L, m, s[m]);
          return L;
        };
      const ee = (L) =>
        e.createElement(
          'svg',
          te({ viewBox: '64 64 896 896' }, L),
          e.createElement('path', {
            d: 'M854.6 370.6c-9.9-39.4 9.9-102.2 73.4-124.4l-67.9-3.6s-25.7-90-143.6-98c-117.8-8.1-194.9-3-195-3 .1 0 87.4 55.6 52.4 154.7-25.6 52.5-65.8 95.6-108.8 144.7-1.3 1.3-2.5 2.6-3.5 3.7C319.4 605 96 860 96 860c245.9 64.4 410.7-6.3 508.2-91.1 20.5-.2 35.9-.3 46.3-.3 135.8 0 250.6-117.6 245.9-248.4-3.2-89.9-31.9-110.2-41.8-149.6zm-204.1 334c-10.6 0-26.2.1-46.8.3l-23.6.2-17.8 15.5c-47.1 41-104.4 71.5-171.4 87.6-52.5 12.6-110 16.2-172.7 9.6 18-20.5 36.5-41.6 55.4-63.1 92-104.6 173.8-197.5 236.9-268.5l1.4-1.4 1.3-1.5c4.1-4.6 20.6-23.3 24.7-28.1 9.7-11.1 17.3-19.9 24.5-28.6 30.7-36.7 52.2-67.8 69-102.2l1.6-3.3 1.2-3.4c13.7-38.8 15.4-76.9 6.2-112.8 22.5.7 46.5 1.9 71.7 3.6 33.3 2.3 55.5 12.9 71.1 29.2 5.8 6 10.2 12.5 13.4 18.7 1 2 1.7 3.6 2.3 5l5 17.7c-15.7 34.5-19.9 73.3-11.4 107.2 3 11.8 6.9 22.4 12.3 34.4 2.1 4.7 9.5 20.1 11 23.3 10.3 22.7 15.4 43 16.7 78.7 3.3 94.6-82.7 181.9-182 181.9z',
          }),
        );
      var ne =
          'data:image/svg+xml;base64,',
        ce = Object.defineProperty,
        ae = Object.getOwnPropertySymbols,
        se = Object.prototype.hasOwnProperty,
        oe = Object.prototype.propertyIsEnumerable,
        le = (L, s, m) => (s in L ? ce(L, s, { enumerable: !0, configurable: !0, writable: !0, value: m }) : (L[s] = m)),
        ie = (L, s) => {
          for (var m in s || (s = {})) se.call(s, m) && le(L, m, s[m]);
          if (ae) for (var m of ae(s)) oe.call(s, m) && le(L, m, s[m]);
          return L;
        };
      const Me = (L) =>
        e.createElement(
          'svg',
          ie({ viewBox: '64 64 896 896' }, L),
          e.createElement('path', {
            d: 'M564.7 230.1V803h60l25.2 71.4L756.3 803h131.5V230.1H564.7zm247.7 497h-59.9l-75.1 50.4-17.8-50.4h-18V308.3h170.7v418.8zM526.1 486.9H393.3c2.1-44.9 4.3-104.3 6.6-172.9h130.9l-.1-8.1c0-.6-.2-14.7-2.3-29.1-2.1-15-6.6-34.9-21-34.9H287.8c4.4-20.6 15.7-69.7 29.4-93.8l6.4-11.2-12.9-.7c-.8 0-19.6-.9-41.4 10.6-35.7 19-51.7 56.4-58.7 84.4-18.4 73.1-44.6 123.9-55.7 145.6-3.3 6.4-5.3 10.2-6.2 12.8-1.8 4.9-.8 9.8 2.8 13 10.5 9.5 38.2-2.9 38.5-3 .6-.3 1.3-.6 2.2-1 13.9-6.3 55.1-25 69.8-84.5h56.7c.7 32.2 3.1 138.4 2.9 172.9h-141l-2.1 1.5c-23.1 16.9-30.5 63.2-30.8 65.2l-1.4 9.2h167c-12.3 78.3-26.5 113.4-34 127.4-3.7 7-7.3 14-10.7 20.8-21.3 42.2-43.4 85.8-126.3 153.6-3.6 2.8-7 8-4.8 13.7 2.4 6.3 9.3 9.1 24.6 9.1 5.4 0 11.8-.3 19.4-1 49.9-4.4 100.8-18 135.1-87.6 17-35.1 31.7-71.7 43.9-108.9L497 850l5-12c.8-1.9 19-46.3 5.1-95.9l-.5-1.8-108.1-123-22 16.6c6.4-26.1 10.6-49.9 12.5-71.1h158.7v-8c0-40.1-18.5-63.9-19.2-64.9l-2.4-3z',
          }),
        );
      var fe =
          'data:image/svg+xml;base64,',
        ge = r(95568),
        ue = { github: N.r, weibo: u, twitter: X, gitlab: p, facebook: T, zhihu: Me, yuque: ee, linkedin: R },
        q = function (s) {
          var m = s.icon,
            ye = s.link,
            me = (0, ge.Z)(),
            Ae = (0, e.useMemo)(
              function () {
                return { Icon: ue[m], link: ye };
              },
              [m, ye],
            );
          return e.createElement(
            'a',
            {
              className: 'dumi-default-icon',
              'data-dumi-tooltip': me.formatMessage({ id: 'header.social.'.concat(m) }),
              'data-dumi-tooltip-bottom': !0,
              target: '_blank',
              href: Ae.link,
              rel: 'noreferrer',
            },
            e.createElement(Ae.Icon, null),
          );
        },
        re = q;
    },
    30011: function (j, h, r) {
      'use strict';
      r.d(h, {
        r: function () {
          return T;
        },
      });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (N, y, D) => (y in N ? I(N, y, { enumerable: !0, configurable: !0, writable: !0, value: D }) : (N[y] = D)),
        Q = (N, y) => {
          for (var D in y || (y = {})) O.call(y, D) && z(N, D, y[D]);
          if (v) for (var D of v(y)) w.call(y, D) && z(N, D, y[D]);
          return N;
        };
      const T = (N) =>
        e.createElement(
          'svg',
          Q({ viewBox: '64 64 896 896' }, N),
          e.createElement('path', {
            d: 'M884 256h-75c-5.1 0-9.9 2.5-12.9 6.6L512 654.2 227.9 262.6c-3-4.1-7.8-6.6-12.9-6.6h-75c-6.5 0-10.3 7.4-6.5 12.7l352.6 486.1c12.8 17.6 39 17.6 51.7 0l352.6-486.1c3.9-5.3.1-12.7-6.4-12.7z',
          }),
        );
      var E =
        'data:image/svg+xml;base64,';
    },
    19444: function (j, h, r) {
      'use strict';
      r.d(h, {
        r: function () {
          return T;
        },
      });
      var e = r(70079),
        I = Object.defineProperty,
        v = Object.getOwnPropertySymbols,
        O = Object.prototype.hasOwnProperty,
        w = Object.prototype.propertyIsEnumerable,
        z = (N, y, D) => (y in N ? I(N, y, { enumerable: !0, configurable: !0, writable: !0, value: D }) : (N[y] = D)),
        Q = (N, y) => {
          for (var D in y || (y = {})) O.call(y, D) && z(N, D, y[D]);
          if (v) for (var D of v(y)) w.call(y, D) && z(N, D, y[D]);
          return N;
        };
      const T = (N) =>
        e.createElement(
          'svg',
          Q({ viewBox: '64 64 896 896' }, N),
          e.createElement('path', {
            d: 'M511.6 76.3C264.3 76.2 64 276.4 64 523.5 64 718.9 189.3 885 363.8 946c23.5 5.9 19.9-10.8 19.9-22.2v-77.5c-135.7 15.9-141.2-73.9-150.3-88.9C215 726 171.5 718 184.5 703c30.9-15.9 62.4 4 98.9 57.9 26.4 39.1 77.9 32.5 104 26 5.7-23.5 17.9-44.5 34.7-60.8-140.6-25.2-199.2-111-199.2-213 0-49.5 16.3-95 48.3-131.7-20.4-60.5 1.9-112.3 4.9-120 58.1-5.2 118.5 41.6 123.2 45.3 33-8.9 70.7-13.6 112.9-13.6 42.4 0 80.2 4.9 113.5 13.9 11.3-8.6 67.3-48.8 121.3-43.9 2.9 7.7 24.7 58.3 5.5 118 32.4 36.8 48.9 82.7 48.9 132.3 0 102.2-59 188.1-200 212.9a127.5 127.5 0 0 1 38.1 91v112.5c.8 9 0 17.9 15 17.9 177.1-59.7 304.6-227 304.6-424.1 0-247.2-200.4-447.3-447.5-447.3z',
          }),
        );
      var E =
        'data:image/svg+xml;base64,';
    },
    45466: function (j, h, r) {
      var e = r(16487);
      function I(v) {
        if (Array.isArray(v)) return e(v);
      }
      (j.exports = I), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    44114: function (j) {
      function h(r) {
        if (r === void 0) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
        return r;
      }
      (j.exports = h), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    7122: function (j, h, r) {
      var e = r(72904),
        I = r(8228),
        v = r(75745);
      function O(w) {
        var z = I();
        return function () {
          var T = e(w),
            E;
          if (z) {
            var N = e(this).constructor;
            E = Reflect.construct(T, arguments, N);
          } else E = T.apply(this, arguments);
          return v(this, E);
        };
      }
      (j.exports = O), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    72904: function (j) {
      function h(r) {
        return (
          (j.exports = h =
            Object.setPrototypeOf
              ? Object.getPrototypeOf.bind()
              : function (I) {
                  return I.__proto__ || Object.getPrototypeOf(I);
                }),
          (j.exports.__esModule = !0),
          (j.exports.default = j.exports),
          h(r)
        );
      }
      (j.exports = h), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    65523: function (j, h, r) {
      var e = r(37745);
      function I(v, O) {
        if (typeof O != 'function' && O !== null) throw new TypeError('Super expression must either be null or a function');
        (v.prototype = Object.create(O && O.prototype, { constructor: { value: v, writable: !0, configurable: !0 } })),
          Object.defineProperty(v, 'prototype', { writable: !1 }),
          O && e(v, O);
      }
      (j.exports = I), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    8228: function (j) {
      function h() {
        if (typeof Reflect == 'undefined' || !Reflect.construct || Reflect.construct.sham) return !1;
        if (typeof Proxy == 'function') return !0;
        try {
          return Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {})), !0;
        } catch (r) {
          return !1;
        }
      }
      (j.exports = h), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    52748: function (j) {
      function h(r) {
        if ((typeof Symbol != 'undefined' && r[Symbol.iterator] != null) || r['@@iterator'] != null) return Array.from(r);
      }
      (j.exports = h), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    16207: function (j) {
      function h() {
        throw new TypeError(`Invalid attempt to spread non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      (j.exports = h), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    75745: function (j, h, r) {
      var e = r(26851).default,
        I = r(44114);
      function v(O, w) {
        if (w && (e(w) === 'object' || typeof w == 'function')) return w;
        if (w !== void 0) throw new TypeError('Derived constructors may only return object or undefined');
        return I(O);
      }
      (j.exports = v), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    37745: function (j) {
      function h(r, e) {
        return (
          (j.exports = h =
            Object.setPrototypeOf
              ? Object.setPrototypeOf.bind()
              : function (v, O) {
                  return (v.__proto__ = O), v;
                }),
          (j.exports.__esModule = !0),
          (j.exports.default = j.exports),
          h(r, e)
        );
      }
      (j.exports = h), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    79664: function (j, h, r) {
      var e = r(45466),
        I = r(52748),
        v = r(91229),
        O = r(16207);
      function w(z) {
        return e(z) || I(z) || v(z) || O();
      }
      (j.exports = w), (j.exports.__esModule = !0), (j.exports.default = j.exports);
    },
    22484: function (j, h) {
      'use strict';
      var r = function (T, E, N, y) {
        function D(x) {
          return x instanceof N
            ? x
            : new N(function (f) {
                f(x);
              });
        }
        return new (N || (N = Promise))(function (x, f) {
          function M(S) {
            try {
              p(y.next(S));
            } catch (b) {
              f(b);
            }
          }
          function i(S) {
            try {
              p(y.throw(S));
            } catch (b) {
              f(b);
            }
          }
          function p(S) {
            S.done ? x(S.value) : D(S.value).then(M, i);
          }
          p((y = y.apply(T, E || [])).next());
        });
      };
      function e(T) {
        let E = 0,
          N = 0,
          y = T;
        do (E += y.offsetTop || 0), (N += y.offsetLeft || 0), (y = y.offsetParent);
        while (y);
        return { top: E, left: N };
      }
      class I {
        constructor(E) {
          this.element = E;
        }
        getHorizontalScroll() {
          return this.element.scrollLeft;
        }
        getVerticalScroll() {
          return this.element.scrollTop;
        }
        getMaxHorizontalScroll() {
          return this.element.scrollWidth - this.element.clientWidth;
        }
        getMaxVerticalScroll() {
          return this.element.scrollHeight - this.element.clientHeight;
        }
        getHorizontalElementScrollOffset(E, N) {
          return e(E).left - e(N).left;
        }
        getVerticalElementScrollOffset(E, N) {
          return e(E).top - e(N).top;
        }
        scrollTo(E, N) {
          (this.element.scrollLeft = E), (this.element.scrollTop = N);
        }
      }
      class v {
        constructor() {
          this.element = window;
        }
        getHorizontalScroll() {
          return window.scrollX || document.documentElement.scrollLeft;
        }
        getVerticalScroll() {
          return window.scrollY || document.documentElement.scrollTop;
        }
        getMaxHorizontalScroll() {
          return (
            Math.max(
              document.body.scrollWidth,
              document.documentElement.scrollWidth,
              document.body.offsetWidth,
              document.documentElement.offsetWidth,
              document.body.clientWidth,
              document.documentElement.clientWidth,
            ) - window.innerWidth
          );
        }
        getMaxVerticalScroll() {
          return (
            Math.max(
              document.body.scrollHeight,
              document.documentElement.scrollHeight,
              document.body.offsetHeight,
              document.documentElement.offsetHeight,
              document.body.clientHeight,
              document.documentElement.clientHeight,
            ) - window.innerHeight
          );
        }
        getHorizontalElementScrollOffset(E) {
          return (window.scrollX || document.documentElement.scrollLeft) + E.getBoundingClientRect().left;
        }
        getVerticalElementScrollOffset(E) {
          return (window.scrollY || document.documentElement.scrollTop) + E.getBoundingClientRect().top;
        }
        scrollTo(E, N) {
          window.scrollTo(E, N);
        }
      }
      const O = {
          elements: [],
          cancelMethods: [],
          add: (T, E) => {
            O.elements.push(T), O.cancelMethods.push(E);
          },
          remove: (T, E) => {
            const N = O.elements.indexOf(T);
            N > -1 && (E && O.cancelMethods[N](), O.elements.splice(N, 1), O.cancelMethods.splice(N, 1));
          },
        },
        w = typeof window != 'undefined',
        z = {
          cancelOnUserAction: !0,
          easing: (T) => --T * T * T + 1,
          elementToScroll: w ? window : null,
          horizontalOffset: 0,
          maxDuration: 3e3,
          minDuration: 250,
          speed: 500,
          verticalOffset: 0,
        };
      function Q(T, E = {}) {
        return r(this, void 0, void 0, function* () {
          if (w) {
            if (!window.Promise) throw "Browser doesn't support Promises, and animated-scroll-to depends on it, please provide a polyfill.";
          } else
            return new Promise((o) => {
              o(!1);
            });
          let N,
            y,
            D,
            x = Object.assign(Object.assign({}, z), E);
          const f = x.elementToScroll === window,
            M = !!x.elementToScroll.nodeName;
          if (!f && !M) throw 'Element to scroll needs to be either window or DOM element.';
          const i = f ? document.documentElement : x.elementToScroll;
          getComputedStyle(i).getPropertyValue('scroll-behavior') === 'smooth'
            && console.warn(`${i.tagName} has "scroll-behavior: smooth" which can mess up with animated-scroll-to's animations`);
          const S = f ? new v() : new I(x.elementToScroll);
          if (T instanceof Element) {
            if (((D = T), M && (!x.elementToScroll.contains(D) || x.elementToScroll.isSameNode(D))))
              throw 'options.elementToScroll has to be a parent of scrollToElement';
            (N = S.getHorizontalElementScrollOffset(D, x.elementToScroll)), (y = S.getVerticalElementScrollOffset(D, x.elementToScroll));
          } else if (typeof T == 'number') (N = S.getHorizontalScroll()), (y = T);
          else if (Array.isArray(T) && T.length === 2)
            (N = T[0] === null ? S.getHorizontalScroll() : T[0]), (y = T[1] === null ? S.getVerticalScroll() : T[1]);
          else
            throw `Wrong function signature. Check documentation.
Available method signatures are:
  animateScrollTo(y:number, options)
  animateScrollTo([x:number | null, y:number | null], options)
  animateScrollTo(scrollToElement:Element, options)`;
          (N += x.horizontalOffset), (y += x.verticalOffset);
          const b = S.getMaxHorizontalScroll(),
            P = S.getHorizontalScroll();
          N > b && (N = b);
          const k = N - P,
            U = S.getMaxVerticalScroll(),
            W = S.getVerticalScroll();
          y > U && (y = U);
          const B = y - W,
            R = Math.abs(Math.round((k / 1e3) * x.speed)),
            C = Math.abs(Math.round((B / 1e3) * x.speed));
          let A = R > C ? R : C;
          return (
            A < x.minDuration ? (A = x.minDuration) : A > x.maxDuration && (A = x.maxDuration),
            new Promise((o, g) => {
              k === 0 && B === 0 && o(!0), O.remove(S.element, !0);
              let d;
              const V = () => {
                n(), cancelAnimationFrame(d), o(!1);
              };
              O.add(S.element, V);
              const J = (H) => H.preventDefault(),
                X = x.cancelOnUserAction ? V : J,
                K = x.cancelOnUserAction ? { passive: !0 } : { passive: !1 },
                t = ['wheel', 'touchstart', 'keydown', 'mousedown'],
                n = () => {
                  t.forEach((H) => {
                    S.element.removeEventListener(H, X, K);
                  });
                };
              t.forEach((H) => {
                S.element.addEventListener(H, X, K);
              });
              const a = Date.now(),
                G = () => {
                  var H = Date.now() - a,
                    Y = H / A;
                  const u = Math.round(P + k * x.easing(Y)),
                    c = Math.round(W + B * x.easing(Y));
                  H < A && (u !== N || c !== y)
                    ? (S.scrollTo(u, c), (d = requestAnimationFrame(G)))
                    : (S.scrollTo(N, y), cancelAnimationFrame(d), n(), O.remove(S.element, !1), o(!0));
                };
              d = requestAnimationFrame(G);
            })
          );
        });
      }
      h.Z = Q;
    },
  },
]);
