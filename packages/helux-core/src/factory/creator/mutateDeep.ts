import { nodupPush, noop } from '@helux/utils';
import { createDraft, finishDraft, IOperateParams } from 'limu';
import { OP_KEYS, REACTIVE_META_KEY } from '../../consts';
...
export function prepareDeepMutate(opts: IPrepareMutateOpts) {
  ...
  const { forAtom, rawState, sharedKeyStr } = internal;
  const mutateCtx = newMutateCtx(setFactoryOpts);
  const draftRoot = createDraft(rawState, {
    sourceId: sharedKeyStr,
    disableProxy: internal.disableProxy,
...
export function execFinish(commitOpts: ICommitOpts, draftRoot: any, draftNode: any, partial?: Dict) {
  ...
  DRAFT_ROOT.del();
  MUTATE_CTX.del();
  // if disableProxy = false, here return a structural shared obj generated by limu
  const nextState = finishDraft(draftRoot);
  noop(finishDraft, REACTIVE_META_KEY);
  commitOpts.state = nextState;

  // 做了赋值操作但都是相同的值，结束后的草稿引用不会改变，这里直接结束 finish 流程
  // 例如 { a: { b: 1 } }
  // 某个地方操作 draft=>draft.a.b = 1; // 此操作并没有引起改变
  if (nextState === internal.rawState && !disableProxy) {
    return;
  }
  ...
  internal.lifecycle.afterCommit({ snap: nextState, moduleName, sn, desc, from, disableProxy });
}
