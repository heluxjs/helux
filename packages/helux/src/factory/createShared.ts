import type { Atom, Call, Dict, Fn, ICreateOptionsType, NextState, SetAtom, SetState, SharedObject } from '../types';
...
export function createSharedLogic<T extends Dict = Dict>(
  ...
  return {
    ...
    call: (srvFn: Fn, ...args: any[]): NextState<any> => {
      ...
      return Promise.resolve(srvFn({ state, draft, setState, args })).then((partialState) => {
