...
import { attachInsProxyState } from '../../helpers/ins';
...
export function checkStateVer(insCtx: InsCtxDef, options: IUseSharedOptions) {
  const {
    ver,
...
export function clearInsCtx(insCtx: InsCtxDef) {
  const { id, globalId, insKey } = insCtx;
 ...
 */
export function isSharedKeyChanged<T extends Dict = Dict>(insCtx: InsCtxDef, sharedState: T) {
