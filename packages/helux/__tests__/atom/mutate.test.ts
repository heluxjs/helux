...
describe('create atom mutate', () => {
  ...
  test('mutate dict(fn), change draft', async () => {
    const [numAtom, setAtom] = atom(1);
    const [bAtom] = atom({ a: 0, b: 0 }, {
      mutate: {
        a: (draft) => { draft.val.a = numAtom.val + 10 },
        b: (draft) => { draft.val.b = numAtom.val + 100 },
      }
    });
  ...
  test('mutate dict(fnItem), change draft', async () => {
    ...
    const [bAtom] = atom({ a: 0, b: 0 }, {
      mutate: {
        a: {
          fn: (draft) => { draft.val.a = numAtom.val + 10 }
        },
        b: {
          fn: (draft) => { draft.val.b = numAtom.val + 100 }
        },
      }
    });
    expect(bAtom.val.a).toBe(11);
